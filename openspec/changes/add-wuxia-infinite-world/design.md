...
本设计是 2026-09-24 的实施前规划；动机与范围见 [proposal.md](proposal.md)。以下“现状”来自项目源码，“方案”是待实现的行为，没有把方案描述为现有能力。

前置条件：先完成 [refactor-ai-service](../refactor-ai-service/design.md) 并验收 NPC 兼容性。以下 AI 服务路径使用重构后的 `ai/`；公共通信、模型调用和业务注册由前置变更提供，场景队列与发布流程由本变更实现。
...
- 先有可信的空间关系，再有 AI 文学描写；默认模板足以独立游玩。
...
- 允许的材质、植被和静态装饰；禁止出现的奖励承诺、NPC 对话、玩家个人经历。

不发送玩家姓名、武功、聊天历史；不使用 NPC 的记忆/RAG 流程。场景名、地点方向不能由模型重命名。动态天气、当前敌人、物品列表和离境门由游戏另行呈现，正文避免声称“永远下雨”“无人存在”等动态事实。
...
发布前检查字段集合、类型、长度、控制字符、ANSI/终端控制、代码围栏、已知危险标记及事实 ID 子集，拒绝模型回传的额外命令、奖励和结构字段。世界键和摘要从任务记录赋值，不相信模型回传。普通自然语言的全部事实无法靠这些检查证明；需用禁止项、固定事实和抽样审读控制质量，明显矛盾结果隔离，不能宣称语义验证百分之百可靠。首期不增加第二个“审稿模型”调用。
