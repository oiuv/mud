...
正在实施 `evolve-ai-capability-runtime`。已建立独立部署、结构化模型响应、通用 Agent 循环、统一 Tool/Skill 执行和 Hook 生命周期，NPC、内部摘要及世界生成已迁入；离线回归和合成资料真实模型联调通过。源码调查、主 Agent 接入、部分旧诊断入口迁移、跨平台及实服验收仍待完成，不能将设计文档视为全部已实现功能。迁移基线为 `2f8192a5`，运行时基础提交为 `0da0c7ab`，无限世界剩余玩法和效果验收仍独立跟踪。
...
`verify_business_agents.py --execute` 仅使用临时角色、知识、历史、冻结清单和业务库，最多 7 次模型调用，验证摘要、共享工具问答、重放和世界发布；默认不执行。输出含合成答案供人工审读，数值检查兼容中文数字，不把“二百”误判成缺少 `200`。旧 `diagnose_chat`/检索维护脚本的运行时迁移仍须继续审查，不作为新增业务绕过统一边界的范例。
