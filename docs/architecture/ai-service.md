...
源码工具支持可选 `SOURCE_SCOPES_FILE`，默认不加载任何范围；配置根目录独立于本 MUDLIB。配置装配和发现已完成，NPC 源码调查授权仍待后续任务；示例全部禁用，读取与外发独立判定。详见 [源码只读范围配置](ai-source-access.md)，包括配置字段、证据快照、资源上限及 Windows/Linux 安全专项的验证边界。
