...
| Telnet / Binary | Telnet 推荐使用；Binary 保留旧版兼容；处理拆包、IAC 转义、终端类型、字符集及窗口尺寸 |
...
协议测试覆盖输入与历史、协商与分片、连接超时、重连、迟到消息、即时输入和窗口上报。浏览器测试使用独立配置及本机测试服务器，验证实际 xterm 渲染、中文输入、安全文本、按键、备用屏幕、手机布局和历史保留；截图输出到系统临时目录。
