...
void greeting(object me)
{
    ...
    if (!me->query("startroom") || base_name(environment()) != me->query("startroom"))
        return;

    if (undefinedp(me->query("born")))
    {
        command("say " + me->query("name") + NOR CYN"，你快快在无忧池中洗脱筋骨(" NOR +
                HIY "指令：washto" NOR + CYN ")，然后投胎去吧！" NOR);
    }
    else
    {
        if (wizardp(me))
    ...
    if (undefinedp(me->query("born")) && !objectp(present("heros book", me)))
    {
        // todo 增加新手游戏基本操作教学
        tell_object(me, WHT "\n地藏王在你耳边悄声说道：本座瞧你人不"
                            "错，这里有一本天书，相信你拿去后有用。\n" NOR);
