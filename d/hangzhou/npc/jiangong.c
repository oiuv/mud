...
mixed ask_mine()
{
    ...
    if (me->query("combat_exp") > 300000 && me->query("mark/job_mine2") > 1000)
...
mixed ask_transit()
{
    ...
    if (me->query("combat_exp") > 500000 && me->query("mark/job_transit2") > 1000)
        return "嘿，您不是跟我开玩笑吧，干这活太委屈你了。";
...
int accept_object(object who, object ob)
{
    ...
    if ((amount = who->query("job/hz_cuprum_ore")) > 0)
    {
        ...
        {
            message_vision(CYN "$N" CYN "看了看$n" CYN "递过来的钱，大喝"
                               "道：“就这点？一边儿去"
                               "！”\n" NOR,
                           this_object(), who);
            return 0;
        ...
        message_vision(CYN "$N" CYN "鼻子哼了一声，接过" + ob->name() + NOR + CYN "，道："
                           "“算了，就不追究你了。”\n" NOR, this_object(), who);
        who->delete ("job/hz_cuprum_ore");
    }
    else
    {
        message_vision(CYN "$N" CYN "眉开眼笑，把" + ob->name() + NOR +
                       CYN "接了过来，收入包囊，却不理会$n" CYN "。\n" NOR,
                       this_object(), who);
    }
