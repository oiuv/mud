...
void invocation(object target) {
    ...
    // 第二掌
    ap = me->query_skill("strike") * 3 / 2 + me->query("str") * 5 + me->query("level") * 20;
    dp = target->query_skill("parry") + target->query("dex") * 5 + target->query("level") * 20;
    ...
    // 第三掌
    ap = me->query_skill("strike") * 3 + me->query("str") * 5 + me->query_skill(
        "martial-cognize",
        1
    );
    dp = target->query_skill("force") + target->query("con") * 5 + target->query_skill(
        "martial-cognize",
        1
    );
