...
string query_save_file() {
    if (bb == 1)
        return DATA_DIR + "cchess/"
            +this_player()->query("id")[0..0] + "/" + this_player()->query("id");
    else
...
string long() {
    ...
    if (me->name() != ob->query("first")) {
        ...
        TABLE[table[8]][0][0] = "└";
        TABLE[table[18]][0][0] = "└";
        TABLE[table[2]][16][0] = TABLE[table[4]][16][0] = TABLE[table[6]][16][0] = TABLE[table[12]][16][0] = TABLE[table[14]][16][0] = TABLE[table[16]][16][0] = "┤";
        TABLE[table[2]][0][0] = TABLE[table[4]][0][0] = TABLE[table[6]][0][0] = TABLE[table[12]][0][0] = TABLE[table[14]][0][0] = TABLE[table[16]][0][0] = "├";
        ...
        if (ob->query("name2")) {
            TABLE[table[3]][16][1] = sprintf(
                "%s%s%s",
                "┃",
                " (" + ob->query("first") + ") ",
                HIR "红方" NOR + "所吃棋子："
            );
            TABLE[table[13]][16][1] = sprintf(
                "%s%s%s",
                "┃",
                " (" + msg + ") ",
                HIC "蓝方" NOR + "所吃棋子："
        ...
        if (ob->query("name2")) {
            TABLE[table[5]][0][1] = sprintf(
                "%s%s%s",
                "┃",
                " (" + ob->query("first") + ") ",
                HIR "红方" NOR + "所吃棋子："
            );
            TABLE[table[15]][0][1] = sprintf(
                "%s%s%s",
                "┃",
                " (" + msg + ") ",
                HIC "蓝方" NOR + "所吃棋子："
