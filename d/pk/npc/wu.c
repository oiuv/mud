...
mixed ask_dao() {
    ...
    if (objectp(owner) && owner != this_object()) {
        ...
        if (owner->query("family/family_name") == "碧刀门")
            return "我的宝刀现在是我徒弟" + owner->query("name") + "在用，你要用就去找他吧。";
        else if (owner->query("family/family_name") == "灵鹫宫")
            return "我的宝刀被你们灵鹫宫的" + owner->query("name") + "取去了，你要用就去找他吧。";
        else
            return "我的宝刀现在落到了" + owner->query("name") + "手中，你去找他吧！";
    }
    ob->move(this_object());
