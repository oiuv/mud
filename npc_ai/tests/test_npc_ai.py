...
class Fixture(unittest.TestCase):
    ...
    def server(self, simulated=False):
        ...
        return UDPServer(settings=self.settings, npc_manager=npc), npc
...
class StorageTests(Fixture):
    ...
    def test_many_players_persist_concurrently(self):
        ...
        self.assertEqual(len(server.memory.get_npc_players("npc")), 25)
...
class ChatTests(Fixture):
    ...
    def test_summary_keeps_current_question_and_original_input(self):
        ...
        self.assertEqual(server.history.get_conversation_history("npc", "player", 2)[0]["content"], "问题5")
        self.assertGreater(server.history.get_summary("npc", "player")["through_id"], 0)
    ...
    def test_summary_failure_still_answers_current_question(self):
        ...
        self.assertEqual(server.history.get_summary("npc", "player")["through_id"], 0)
    ...
    def test_capacity_zero_omits_context_but_keeps_archive(self):
        ...
        self.assertEqual(len(server.history.get_conversation_history("npc", "player")), 4)
    ...
    def test_failed_model_does_not_save_or_grow_relationship(self):
        ...
        self.assertEqual(server.memory.get_player_memory("npc", "player"), {})
        self.assertEqual(server.history.get_conversation_history("npc", "player"), [])
    ...
    def test_duplicate_concurrent_requests_run_once(self):
        ...
        self.assertEqual(server.memory.get_player_memory("npc", "player")["total_interactions"], 1)
...
class EdgeCaseTests(Fixture):
    ...
    def test_conflicting_ids_cannot_commit_across_concurrent_sessions(self):
        ...
        self.assertEqual(len(server.memory.get_npc_players("npc")), 1)
    ...
    def test_same_session_concurrent_distinct_requests_keep_relationship(self):
        ...
        self.assertTrue(all(result["type"] == "chat" for result in results))
        self.assertEqual(server.memory.get_player_memory("npc", "player")["total_interactions"], 20)
        self.assertEqual(len(server.history.get_conversation_history("npc", "player", 100)), 40)
