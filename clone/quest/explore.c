...
void init_quest(object npc_ob, string qob_name, string zone, string *files) {
    ...
    // 银两和铜板
    env->set_temp("no_search", ([ "/clone/money/coin": 0, "/clone/money/silver": 0, ]));
...
object search_qob(object me) {
    ...
    if (!environment(me)->query("no_fight") && random(100) < 10) {
        ...
        message_vision(
            HIW "\n突然窜出一个黑影，恶狠狠的对$N" HIW "骂道：" + RANK_D->query_rude(me) + "！你在这儿干啥？\n" NOR,
            me
        );
        ob->kill_ob(me);
        return 0;
...
string ask_npc(object knower, object me) {
    ...
    if (!objectp(NPC_OB))
        return CYN "哦，据说" HIY + NPC_NAME + NOR CYN "不知道去哪儿去了，神秘的失踪了。" NOR;
...
string ask_qob(object knower, object me) {
    ...
    if (!objectp(QOB) || !objectp(env = ENV_OB) && !environment(QOB))
        return CYN "据可靠的消息说，" + QOB_NAME + CYN "是再也找不着了，再怎么也是白忙活了。" NOR;

    if (environment(QOB))
        return CYN "好像说是有人已经找到" + QOB_NAME + CYN "了，啧啧，怎么就不是我呢？" NOR;
    ...
    if (me->query("score") >= 2000) {
        ...
        message("vision", WHT + knower->name() + "看了看四周，鬼鬼祟祟的在" +
            me->name() + WHT "耳边说了些什么。\n" NOR,
            environment(me), ({ me }));

        tell_object(me, sort_msg(knower->name() + "看了看四周，鬼鬼祟祟的在你耳边说道：“你可千万"
            "别告诉别人呀！这可是我听到的秘密情报，"
            "今天我不要钱就告诉你了：据说那个东西在一个" +
            exits + "的地方，离咱们这里也不算太远呢。你到那里仔细找找没准会有发现呢！”\n"));
    ...
    return CYN "其实你说这" HIY + QOB_NAME + NOR CYN "东西有啥用呢？能比得上鸡腿和酒袋么？" NOR;
}
...
string query_introduce(object knower) {
    ...
    return CYN "据说" + HIY + NPC_NAME + NOR CYN "正在托人寻找" + HIY + QOB_NAME + NOR CYN "，啧啧，可惜我要看店。" NOR;
}
...
string query_prompt() {
    switch (random(3)) {
        case 0:
            return CYN "最近听来往的客人谈起过『" HIY + name() + NOR CYN "』来。";
        case 1:
            return "噢！楼上住店的那几个客人刚才说什么『" HIY + name() + NOR CYN "』呢。";
        default:
            return "前两天来过几个江湖人士，一看就是高手，他们还说起过『" HIY + name() + NOR CYN "』呢。";
    }
}
...
int npc_accept_object(object me, object who, object ob) {
    ...
    message_sort(HIY "\n$N" HIY "接过" + ob->name() + HIY "，翻来覆去看了看，喜不自胜，一时间是激动万分。半晌才对$n" HIY "道：这位" +
        RANK_D->query_respect(who) + "，实在是感激不尽，这点小意思，务必笑纳！\n" NOR,
        me, who);

    message_vision(HIC "$N" HIC "交给$n" HIC "一些" NOR + YEL "黄金" HIC "作为报酬。\n" NOR, me, who);
    ...
    b = ([
        ...
        "score": score,
        "weiwang": weiwang,
        "prompt": "在寻找" + QOB_NAME + HIC "的过程中，经过锻炼",
    ]);
    GIFT_D->delay_bonus(who, b);

    CHANNEL_D->do_channel(this_object(), "rumor",
        "听说" + who->name(1) + "(" + who->query("id") + ")替" + me->name() + HIM +
        "找到了" + QOB_NAME + HIM "，领到了赏银。" NOR);
