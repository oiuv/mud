...
string ask_npc1(object knower, object me) {
    ...
    if (!objectp(NPC1) && !objectp(NPC2)) {
        ...
        return CYN "嘿嘿，据说" HIY + NPC1_NAME + NOR + CYN "和" HIY + NPC2_NAME + NOR + CYN "哥"
            "俩终于见面了。啧啧，真不错。" NOR;
    }

    if (!objectp(NPC1))
        return CYN "唉…据说" HIY + NPC1_NAME + NOR + CYN "还是被仇家给做掉了，到死也没见着"
            HIY + NPC2_NAME + NOR + CYN "的面。" NOR;

    me->set_temp("trace/" + NPC1_ID, 1);

    return CYN "这人武功不高，可是极重义气，跟他兄弟" HIY + NPC2_NAME + NOR + CYN "失散后，便四处躲避仇家"
...
string ask_npc2(object knower, object me) {
    ...
    if (!objectp(NPC2))
        return CYN "唉…据说那" HIY + NPC2_NAME + NOR + CYN "最后还是被仇家给做掉了，临死前还惦"
