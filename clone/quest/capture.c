...
string ask_npc2(object knower, object me) {
    ...
    if (!objectp(NPC2))
        return CYN "你刚才没听到谣言么？听说那个" HIY + NPC2_NAME + NOR CYN "好象也挂了。你瞧瞧"
            "看，这就是惹是生非的下场。\n" + knower->name() + CYN "接着说道：不过现在又传出了消息，"
            "扰乱六大门派的主谋竟然也不是他，而是一"
...
*/
    ...
    if (!objectp(NPC3))
        return CYN "死了，全死光了。" HIY + NPC3_NAME + NOR CYN "也让人给做了。你算算，先是" HIY + NPC1_NAME + NOR + CYN "，然后是" HIY + NPC2_NAME + NOR + CYN "，现在又轮到他。"
    ...
    return CYN "我说这家伙耐性可真好，等到他同伴全死光了他"
        ...
        "已经直奔" + PLACE3 + "找他去了。\n" + knower->name() + CYN "轻哼一声，接着道：我说吧，人总归会有倒霉"
...
string query_introduce(object knower) {
    ...
    if (objectp(NPC1))
        return CYN "唉呀呀，听说" HIY + NPC1_NAME + NOR + CYN "那家伙不知怎么搞的，居然把江湖上六大门"
            "派全都给惹了，真是有种！\n" + knower->name() + CYN "接着说道：这下一来，六大门派便放出口"
    ...
    if (!objectp(NPC3))
        return CYN "嘿嘿，这下人死光了，也算是太平了。\n" + knower->name() + CYN "白眼一翻道：你算算，先"
            ...
            +NPC3_NAME + NOR + CYN "。\n" + knower->name() + CYN "又接着道：啧啧啧，谁叫他们把该惹的和不"
