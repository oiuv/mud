...
mixed ask_for_qob(object npc2, object qob, string qob_name) {
    ...
    if (objectp(qob) && environment(qob) == npc2 && qob->query_amount()) {
        if (me->query_encumbrance() + qob->weight() / qob->query_amount() <
            ...
            qob->add_amount(-1);
            if (!objectp(ob))
                tell_object(me, CYN + npc2->name() + CYN "突然脸色一变，大叫道：我的" +
                    qob_name + CYN "呢？怎么不见了？\n" NOR);
            ob->move(me, 1);
        } else {
        ...
        message_vision(CYN "$N" CYN "微笑着对$n" CYN "说：不用劳烦你了，这批" +
            qob_name + CYN "已经有人送去了。\n" NOR, npc2, me);
...
string ask_npc1(object knower, object me) {
    ...
    if (!objectp(NPC1))
        return CYN "唉，怎么" HIY + NPC1_NAME + NOR CYN "好端端的，" HIY + QOB_NAME + NOR CYN "没拿到，反而丢了命，真是可怜。" NOR;

    return CYN "哦，他呀，据说正在" + PLACE1 + "，等人给他送去一批" + HIY + QOB_NAME + NOR CYN "呢！" NOR;
}
...
string ask_npc2(object knower, object me) {
    ...
    if (!objectp(NPC2))
        return CYN "听人说" HIY + NPC2_NAME + NOR CYN "居然被人杀了，也不知道惹了哪门子的祸！" NOR;

    if (me->query("score") < 500)
        call_out("do_whisper", 1, knower, me);

    return CYN "哦，他呀，据说正在" + PLACE2 + "，急着等人帮他送货呢！" NOR;
}
...
void do_whisper(object knower, object me) {
    ...
    tell_object(me, WHT + knower->name() + WHT "悄悄的和你说：“可惜你的江湖阅历太低了，他不会给你运。”\n");
    message(
        "vision",
        knower->name() + "在" + me->name() + "的耳边悄悄的说了些什么。\n",
        environment(me),
        ({ me })
    );
}
...
string ask_qob(object knower, object me) {
    ...
    if (!objectp(QOB))
        return CYN "难不成已经给山贼截去了？居然没人知道那" HIY + QOB_NAME + NOR CYN "的下落了。" NOR;

    return CYN "这" HIY + QOB_NAME + NOR CYN + "呀，不过是很普通的一批货物而已嘛！" NOR;
}
...
string query_introduce(object knower) {
    ...
    return CYN "听说" HIY + NPC2_NAME + NOR CYN "急需将一批" + HIY + QOB_NAME + NOR CYN "送到" HIY + NPC1_NAME + NOR CYN "那儿，你有没有兴趣？" NOR;
}
...
string query_prompt() {
    switch (random(3)) {
        case 0:
            return CYN "倒是最近听来往的商人说起『" HIY + name() + NOR CYN "』的事来。";
        case 1:
            return "也没什么大事，只是听说过『" HIY + name() + NOR CYN "』的事罢了。";
        default:
            return "前两天还听人家说起『" HIY + name() + NOR CYN "』的事呢。";
    }
}
...
int npc_accept_object(object me, object who, object ob) {
    ...
    if (ob->is_character() || ob->is_item_make()) {
        message_vision(CYN "$N" CYN "瞪着$n" CYN "，使劲的摇着头道：我要这干什么？\n" NOR,
            me, who);
        return 0;
    }
    ...
    message_vision(CYN "$N" CYN "看了看" + ob->name() + CYN "，对$n" CYN "道：还不错，辛苦你了。这点银子你收下吧。\n" NOR,
        me, who);
    ...
    if (AMOUNT < 1) {
        // 所有的物品全部接收完了，准备结束任务
        message_vision(CYN "$N" CYN "抹了抹头上的汗，道：总算全运到了，这事可真够罗嗦的。\n" NOR,
            me);
...
private void do_notice(object me, object who, object ob) {
    ...
    message_vision(CYN "$N" CYN "点了点货，又对$n" CYN "道：我还需要" +
        chinese_number(AMOUNT) + QOB_UNIT + QOB_NAME +
        CYN "才够。\n" NOR, me, who);
}
