...
string ask_1_for_2() {
    ...
    if (!environment(NPC2))
        return NPC2->name() + "这" + RANK_D->query_rude(NPC) + "不知道溜到哪里去了。";
...
string ask_2_for_1() {
    ...
    if (!environment(NPC1))
        return "他奶奶的，" + NPC2->name() + "这个" + RANK_D->query_rude(NPC) + "！跑了？";
...
string query_introduce(object knower) {
    ...
    return CYN "听说" HIY + NPC1_NAME + NOR CYN "和" + HIY + NPC2_NAME + NOR CYN "在" + PLACE + CYN "发生了纠纷，不知道为什么，要是"
