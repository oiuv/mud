...
void set_attrib() {
    ...
    if (stringp(tname)) {
        if (lvl >= 300) {
            desc += HIG "上面镶嵌着" + tname + HIG "，巧夺天工，美得异乎寻常，让" + name() + HIG "越发显得精致。\n" NOR;
        } else if (lvl >= 200) {
            desc += HIG "上面镶嵌着" + tname + HIG "，几乎没有瑕疵，可谓人间罕见的珍品。\n" NOR;
        } else {
            desc += HIG "上面镶嵌着" + tname + HIG "，衬托出" + name() + HIG + "的不俗，只是有些少瑕疵显得美中不足。\n" NOR;
        }
        set("long", desc);
    }
}
...
string do_wear() {
    ...
    if (me->query("gender") == "女性") {
        msg = HIG "$N" HIG "轻巧巧的戴上了一个$n" HIG "，璨然一笑，映的宝光四射，直令得花儿也憔悴了。\n" NOR;
        if (stringp(tname)) {
            if (point >= 150)
        ...
        msg = HIG "$N" HIG "戴上了一个$n" HIG "，当下意气风发，神完气足。\n" NOR;
        if (stringp(tname)) {
            if (point >= 150)
                msg += HIG "只见" + tname + HIG "闪过"
                    "一道光芒，映射得$N" HIG "如同"
...
int receive_summon(object me) {
    ...
    if (env == environment(me)) {
        ...
        if (env) {
            ...
            message("vision", HIG "突然" + name() + HIG "化作一道" HIG "光芒消失了！\n\n" NOR, env);
        ...
        message_vision(HIG "一道光芒划过，只见$N" HIG "掌中多了一个$n" HIG "！\n\n" NOR,
            me, this_object());
    }
