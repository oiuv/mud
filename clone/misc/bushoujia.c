...
int do_snare(string arg)
{
    ...
    if (query_temp("id") != me->query("id") && query_temp("id") && environment() != me)
        return notify_fail("这是别人放置的捕兽夹，你动它做甚？\n");

    if (environment(me)->query("no_fight") || environment(me)->query("no_quarry"))
    ...
    if (!objectp(ob = present("shi er", me)) || !ob->query_amount())
