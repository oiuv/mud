...
以当前 MUD 的服务为主，避免强耦合，不为通用性额外增加抽象。其他 MUD 优先修改 `HELP_DIR`、`NPC_ROLES_FILE`、`DATA_DIR`、`SKILLS_DIR` 等配置接入；专业提示词已迁为可独立修改的 Skill，不要求复制本库目录或使用相同驱动。源码调查和主 Agent 路由等后续工作尚未完成，见 [架构与迁移说明](../docs/architecture/ai-service.md)。
...
NPC 每请求最多 6 次模型调用（摘要 1 次、问答最多 5 次），共用 80 秒以内的期限及 12 次外部请求、16 次工具尝试上限，包含失败调用。角色配置的 `knowledge_threshold` 是检索下限，模型不能调低；值为 1 时禁用召回。只有验证完成且事务提交成功才写入历史、关系和成功去重；澄清、未完成及失败不增加关系值。世界仍是每次队列尝试最多一次模型调用、最多三次持久尝试，旧正文不会因迁移重生成。
...
`python ai/scripts/verify_business_agents.py` 同样默认只显示计划；授权后加 `--execute`，在临时库联调 NPC 检索问答、摘要、成功重放和世界正文发布，整批最多 7 次模型调用，只发送合成资料，不连接正式业务库。报告同时列出文本、终态、耗时与用量，自动数值检查不替代人工审读。
...
诊断脚本经统一 Runner 和模型适配执行，最多调用模型一次、禁止工具与委派，并记录 Hook 和实际用量。默认发送一句简短问题，也可在命令末尾传入自定义问题；不加载知识库、人设或玩家历史，不保存对话。简短请求成功后，再用 `scripts/test_client.py chat "li bai" "武当派如何拜师？"` 测试已启动服务的完整流程；该客户端会按正常规则保存测试玩家的对话。
...
默认读取 `.env` 和当前知识库索引，通过统一 `knowledge.search` 工具复用混合检索（BM25 + 向量召回、RRF 融合、按配置重排）。单次查询最多 2 个外部请求、1 个工具步骤；工具执行上限 30 秒，并受 `REQUEST_TIMEOUT` 的剩余时间约束。`--bm25` 同样经过工具权限与 Hook，但禁止任何外部请求。脚本不调用问答模型、不写玩家对话；检索内容会先按公共资料范围过滤。
...
`performance_test.py` 默认只测本地 BM25；`--remote` 的 6 个问题共用整批预算，最多 12 个外部请求、6 个工具步骤。
`benchmark_cache.py` 通过同一工具的受信任向量诊断模式测试缓存；3 轮共 9 个工具步骤，最多 9 个外部请求（正常缓存命中时只有 3 次）。先完成索引构建；两者均输出实际累计计数并释放所持有的客户端。
