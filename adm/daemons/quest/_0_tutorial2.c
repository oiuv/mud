...
{
    return player->isSolved(F_QUEST("_0_tutorial1"));
}
...
string *getAssignMessage()
{
    // $ME為NPC, $YOU為player
    string *msg = ({
        "$ME对$YOU说到：" HIG "打工挣的辛苦钱，都随身携带也非常不方便，最好能存起来。\n" NOR,
        "$ME对$YOU说到：" HIG "去钱庄存钱试试，让自己的存款超过白银二十两。\n" NOR,
        "$ME对$YOU说到：" HIG "如果你没有这么多钱，可以继续去铁匠铺打铁挣钱。\n" NOR,
        "$ME对$YOU说到：" HIG "游戏中的货币包括黄金、白银和铜钱，兑换比例可以去钱庄试试。\n" NOR,
    });
    return msg;
}
...
{
    return player->query("balance") >= 2000;
}
...
string *getRewardMessage()
{
    // $ME為NPC, $YOU為player
    string *msg = ({
        CYN "$ME对$YOU说到：" HIG "干的漂亮，钱不是万能的，但行走江湖没钱是万万不能的。\n" NOR,
    });
    return msg;
}
