...
void start_quest()
{
    ...
    CHANNEL_D->do_channel(find_object(QUEST_D), "sys", "进程(EXPLORE)" HIR "利用" + qob_name->name() + HIR "和" + npc->name() + HIR "创建了一个任务。");
}
...
void startup()
{
    ...
    if (!query_heart_beat())
        CHANNEL_D->do_channel(find_object(QUEST_D), "sys", "进程(EXPLORE)启动了。");
