...
void start_quest()
{
    ...
    CHANNEL_D->do_channel(find_object(QUEST_D), "sys", "进程(SUPPLY)在" + env->short(1) + HIR "地点利用" + npc->name() + HIR "创建了一个任务。");
...
void startup()
{
    ...
    if (!query_heart_beat())
        CHANNEL_D->do_channel(find_object(QUEST_D), "sys", "进程(SUPPLY)启动了。");
