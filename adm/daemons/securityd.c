...
int valid_write(string file, mixed user, string func) {
    ...
    if (func == "save_object") {
        if ((stringp(user) ? sscanf(user, "/clone/%*s")
            ...
            sscanf(
                file,
                "/data/%*s"
            ) && (file == (string)user->query_save_file() + __SAVE_EXTENSION__ || file == (string)user->query_save_file())) {
    ...
    if (path[0] == "world" && (explode(
        file,
        "."
    )[<1] == "ansi" || file == base_name(user) + ".html"))
        return 1;
