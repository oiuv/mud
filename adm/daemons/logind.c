...
void logon(object ob)
{
    ...
    printf("目前共有 " CYN "%d" NOR " 位巫师、" CYN "%d" NOR " 位玩家在线上，以及 " CYN "%d" NOR " 位使"
...
private void get_id(string arg, object ob)
{
    ...
    if (arg == "new" || arg == "none" || arg == "guest" || arg == "cancel")
    {
...
private void check_ok(object ob)
{
    ...
    {
        write("你把正在聊天的ID踢了出去。\n");
        MESSAGE_D->user_logout(user, user->name(1) + "从" + query_ip_number(ob) + "连线进入世界，把你踢了出去。\n");
    }
...
private void confirm_ad_password(string pass, object ob)
{
    ...
    if (crypt(pass, old_pass) != old_pass)
    {
        write(HIR "\n您两次输入的管理密码不同，请重新设定一次" HIW "管理密码" HIR "：\n" NOR);
        input_to("new_ad_password", 1, ob);
        return;
    }
...
private void confirm_password(string pass, object ob)
{
    ...
    if (crypt(pass, old_pass) != old_pass)
    {
        write(HIR "\n您两次输入的普通密码不同，请重新设定一次" HIY "普通密码" HIR "：\n" NOR);
        input_to("new_password", 1, ob);
        return;
    }
