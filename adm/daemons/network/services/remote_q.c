...
void incoming_request(mapping info)
{
    ...
    if (info["NAME"])
    {
        ...
        {
            (AUX_PATH + "affirmation_a")->send_affirmation_a(info["HOSTADDRESS"], info["PORTUDP"], "remote_q@" + Mud_name(), info["SOURCE_ID"], sprintf("%s没有这个人(%s)。", LOCAL_MUD_NAME(), info["TARGET_ID"]), "remote_q");
            return;
        }
             ...
             is_sub(sid, no_emote)))
        {
            ...
            {
                (AUX_PATH + "affirmation_a")->send_affirmation_a(info["HOSTADDRESS"], info["PORTUDP"], "remote_q@" + Mud_name(), info["SOURCE_ID"], sprintf("%s(%s@%s)不接受你的表情动作。", ob->name(1), capitalize(ob->query("id")), Mud_name()), "remote_q");
                return;
            }
        }
        ...
        DNS_MASTER->send_udp(rhost["HOSTADDRESS"], rhost["PORTUDP"],
                             "@@@" + "remote_a" +
                                 ...
                                 "||MSG:" + info["MSG"] +
                                 "||TARGET:" + sprintf("mud=%s name=%s id=%s age=%d gender=%s respect=%s rude=%s ", Mud_name(), ob->name(1), ob->query("id"), ob->query("age"), ob->query("gender"), RANK_D->query_respect(ob), RANK_D->query_rude(ob)) +
                                 "@@@\n");
    }
}
