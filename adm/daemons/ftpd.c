...
void remove() {
    if (number_of_users && this_player(1) && wizhood(this_player()) != "(admin)")
...
string ls(string path, int column, int fd) {
    ...
    for (i = 0; i < s; i++) {
        /* process timestamp */
        tmp2 = ctime((xfiles[i])[2]); /* get last modified timestamp */ if ((xfiles[i])[2] + SECS_IN_YEAR < time()) {
            /* MMM DD  YYYY */
        ...
        j = (xfiles[i])[1]; /* get filesize */ if (j == -2) {
            /* directory */
...
void data_write_callback(int fd) {
    ...
    if (socket_info[fd][TYPE] == STRING) {
        while (((ret_val = socket_write(
            fd,
            socket_info[fd][DATA][pos..(pos + BLOCK_SIZE - 1)]
        )) >= 0) || (ret_val == EECALLBACK)) {
            pos += BLOCK_SIZE;
        ...
        while (((ret_val = socket_write(fd, tmp)) >= 0) || (ret_val == EECALLBACK)) {
            pos += length;
            if (pos >= socket_info[fd][LEN]) {
                socket_write(socket_info[fd][PARENT_FD], "226 Transfer complete.\n");
                socket_close(fd);
...
protected void data_conn(int fd, string mess, string name, int type) {
    ...
    if (type == STRING || (type == FILE && UTYPE == STRING)) {
    ...
    if ((ret = socket_connect(new_fd, sprintf(
        "%s %d",
    ...
    socket_info[new_fd] = ([ DATA: (type == STRING ? replace_string(mess, "\n", "\r\n") : mess),
        POS: rest_pos,
        PARENT_FD: fd,
        TYPE: type,
...
void data_close_callback(int fd) {
    ...
    if (socket_info[fd][TYPE] == DOWNLOAD) {
        if (socket_info[fd][APPEND] != 1)
            ...
            pos = file_size(socket_info[fd][PATH]);
...
void in_read_callback(int fd, string str) {
    ...
    if (undefinedp(socket_info[fd][CONNECTED]) || !socket_info[fd][CONNECTED])
        return;
