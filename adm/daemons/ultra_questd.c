...
public mixed ask_information(object me, object who, string topic) {
    ...
    if (topic == "线索" || topic == "clue") {
        // 询问目标为上一个提供信息的人
        if (who->query("ultra_quest/npc") == me->name())
            return "我不是让你去" + npc_place + "找" HIY + next_npc + NOR + CYN "么？老来烦我做甚"
                "。" NOR;
        ...
        if (who->query("ultra_quest/quest/msg"))
            return "我不是让你" + who->query("ultra_quest/quest/msg") + "吗？还罗嗦什么？";
...
public void give_quest(object me) {
    ...
    switch (type) {
        case "kill":
            msg = random(2) ? "你帮我杀了" HIR + obj + NOR + WHT "那厮，再提他的头来见我" :
        ...
        case "find":
            msg = random(2) ? "我现在正急需" + obj + NOR + WHT "，只好劳烦劳烦阁下了" :

            "倘若你能帮我找到" + obj + NOR + WHT "，或许我可以提供些线索";

            quest_msg = "帮我找到" + obj;
            break;

        case "send":
            msg = random(2) ? "你帮我把这封信送到" HIY + obj + NOR + WHT "手中，然后拿" HIW
                "回执" NOR + WHT "来见我" :
        ...
        case "give":
            msg = random(2) ? "你帮我把这个包裹送到" HIY + obj + NOR + WHT "那里，看看他"
                "有什么反应" :
...
void check_going(object me, object who) {
    ...
    if (!objectp(me)
        ...
        switch (who->query_temp("dest_npc")) {
            case 0:
                msg = "你回头张望间，却发现那" + who->query("ultra_quest/next_npc") + "竟不知何时不见了踪影。\n你心中暗道"
                    "：可千万别让线索断掉了，得赶紧将他"
                    "找回来。";
                break;

            case 1:
                msg = "你心中颇为焦急：咦？" + who->query("ultra_quest/next_npc") + "那家伙到底上哪去了。";
                break;

            case 2:
                msg = "你心底直骂：什么时候了，那该死的" + who->query("ultra_quest/next_npc") + "怎么还不滚出来。";
                break;
    ...
    if (environment(me)->query("short") == place) {
        ...
        if (stringp(last_place = me->query_temp("last_place"))
            ...
            switch (random(5)) {
                ...
                case 1:
                    msg = "只听$N" HIW "喘息道：" + who->name() + HIW "，走慢点…走慢…累死我了。";
                    break;
                ...
                case 3:
                    msg = "$N" HIW "忽然脸色惨白，道：" + who->name() + HIW "，劳烦停…停一"
                        "会，我走岔气了。";
                    break;
...
public void give_gift(object me) {
    ...
    // 奖励的底数
    gift = me->query("ultra_quest/gift/point") + me->query("ultra_quest/finish");
...
*/
    ...
    switch (random(5)) {
        case 0:
            msg = "听人说，好象" + npc_place + "的" + next_npc + "就有" + ma + "要找的东"
                "西，也不知是真是假";
            break;
        ...
        case 2:
            msg = "不知" + ma + "认识" + npc_place + "的" + next_npc + "吗？传闻你要找的"
                "那东西就在他身上";
            break;

        case 3:
            msg = "不清楚" + ma + "是否认识" + npc_place + "的" + next_npc + "，那东西十有八九"
                "都跟他有关";
            break;

        default:
            msg = "我想想…" + ma + "去找" + npc_place + "的" + next_npc + "问问看吧，去年我听"
                "他说起过这件事";
            break;
    }
    ...
    if (npc == QUESTER) {
        if (me->query("ultra_quest/new_begin"))
            msg = "…嗯…你去" + npc_place + "问问" + next_npc + "看，他那儿"
                "似乎又有些线索";
        else
            msg = "你可以去" + npc_place + "找" + next_npc + "打听打听，他"
                "那似乎有些线索";
    }
    ...
    if (npc == LURCHER) {
        switch (random(3)) {
            case 0:
                msg = "…嘿嘿…你去" + npc_place + "逛逛，那儿有个" + next_npc + "，你问他啥都知道";
                break;

            case 1:
                msg = "…这样吧…你去找" + next_npc + "问问看，那家伙就在" + npc_place + "，可别走丢了";
                break;
