...
public mixed teach_pfm(object who, object ob, mapping b) {
    ...
    if (!intp(free = b["free"]) || free <= 0) {
        ...
        if (who->query("family/family_name") != family)
            return RANK_D->query_respect(who) + "与我" + family + "素无渊源，不知此话从何说起。";
    }
    ...
    if (intp(gongxian = b["gongxian"])
        ...
        return "你为" + who->query("family/family_name") + "作出的贡献不够，这招暂时还不能传你。";
...
public mixed give_item(object who, object ob, mapping b) {
    ...
    if (intp(generation = b["gen"])
        ...
        return "凭你在" + who->query("family/family_name") + "中的地位，还不足以让我将" + obj->name() + CYN
            "交予你。" NOR;
    ...
    if (intp(shen = b["shen"])) {
        ...
        // 如果要求为正神，则作出小于判断
        if (shen > 0 && who->query("shen") < shen)
            return "你目前所做的侠义正事不够，这" + obj->name() + CYN "暂时还不能交"
                "给你。" NOR;
    }
    ...
    if (stringp(temp1 = b["temp1"]) && !who->query(temp1)) {
        if (stringp(tmsg1 = b["tmsg1"]))
            ...
            return "现在某些机缘未到，这" + obj->name() + CYN "暂时还不能交给你。" NOR;
    }
    ...
    if (owner == who)
        return "那" + obj->query("unit") + obj->name() + CYN "不就是你拿着在用么，怎么反倒找我"
            "来了？" NOR;

    if (objectp(owner) && owner != ob) {
        if (!owner->is_character())
            return "那" + obj->query("unit") + obj->name() + CYN "我已经借出去了，你还是隔段时间再"
                "来吧。" NOR;

        if (owner->query("family/family_name") == family)
            return "那" + obj->query("unit") + obj->name() + CYN "现在是我派的" + owner->name() + "在用，你若需要就去找他吧。" NOR;
        else if (owner->query("family/family_name") == who->query("family/family_name"))
            return "那" + obj->query("unit") + obj->name() + CYN "现在是你派的" + owner->name() + "在用，你自己去问他要吧。" NOR;
        else
            return "那" + obj->query("unit") + obj->name() + CYN "现在落在了" + owner->name() + "手中，你去把它取回来吧。" NOR;
    }
    ...
    if (intp(gongxian = b["gongxian"])
        && gongxian > 0
        && who->query("gongxian") < gongxian)
        return "你为" + who->query("family/family_name") + "作出的贡献不够，这" + obj->name() + CYN
            "暂时还不能交给你。" NOR;
    ...
    if (!stringp(msg = b["msg"]))
        msg = "既然这样，那这" + obj->query("unit") + obj->name() + CYN "你就暂时拿去吧。" NOR;
