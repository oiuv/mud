...
mixed create_challenger()
{
    ...
    CHANNEL_D->do_channel(this_object(), "rumor", "听说" + "从" + ob->query("nation") + "来了一个名叫" + ob->name(1) + "的人。");
    next = NOT_READY;
    cob = ob;
...
mixed continue_story(int n)
{
    ...
    case CHALLENGER_IS_FIGHTING:
        if (!rob || environment(rob) != environment(cob))
        {
            ...
            CHANNEL_D->do_channel(this_object(), "rumor", "听说" + accepted + "在和" + cob->query("nation") + "" + cob->name(1) + "交手的时候临阵脱逃，使中原武林蒙羞！");
    ...
    case 11:
        CHANNEL_D->do_channel(this_object(), "rumor", "听说来自" + cob->query("nation") + "的" + cob->name(1) + "在中国耀武扬威了一番后安然的回国去了，真是耻辱。");
        return 0;
    }
...
void accept_for_challenge(object me)
{
    ...
    CHANNEL_D->do_channel(this_object(), "rumor", "听说" + fam + me->name(1) + "挺身而出，接下了" + cob->name(1) + "的挑战！");
    rob = me;
    accepted = me->name(1);
