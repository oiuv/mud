...
mixed query_story_message(int step) {
    if (stop)
        return 0;
    return step < sizeof(story) ? story[step] : continue_story(step - sizeof(story));
}

string show_time() {
    return (random(2)) ? "不知不觉之间，时间到了" + NATURE_D->game_time() + "。" : "岁月如梭，说话间就到了" + NATURE_D->game_time() + "。";
}
...
mixed create_challenger() {
    ...
    CHANNEL_D->do_channel(
        this_object(),
        "rumor",
        "听说" + "从" + ob->query("nation") + "来了一个名叫" + ob->name(1) + "的人。"
    );
    next = NOT_READY;
    cob = ob;
...
mixed master_opinion(int n) {
    ...
    message("vision", HIG + ob->name(1) + "告诉你：你何不出手应战(accept)？扬我中华武林威风！\n",
        u);
...
mixed continue_story(int n) {
    ...
    switch (next) {
        ...
        case CHALLENGER_KILLED_BY_ULTRA:
            ...
            CHANNEL_D->do_channel(
                this_object(),
                "rumor",
                "听说日本鬼子" + cob->name(1) + "被打得他妈都认不出他来了。"
            );
            destruct(cob);
            stop_story();
            return 0;
        ...
        case NO_USER_CAN_RESPOND:
            ...
            CHANNEL_D->do_channel(
                this_object(),
                "rumor",
                "听说来自" + cob->query("nation") + "的" + cob->name(1) + "在中国居然没有对手，真是中原武林的奇耻大辱！"
            );
        ...
        case CHALLENGER_IS_FIGHTING:
            if (!rob || environment(rob) != environment(cob)) {
                ...
                CHANNEL_D->do_channel(
                    this_object(),
                    "rumor",
                    "听说" + accepted + "在和" + cob->query("nation") + "" + cob->name(1) + "交手的时候临阵脱逃，使中原武林蒙羞！"
                );
            ...
            if (rob->query("gender") == "男性")
                msg = fenco[random(sizeof(fenco))] + "大声喊道：" + fenmsg[random(sizeof(fenmsg))];
            else
                msg = menco[random(sizeof(menco))] + "大声喊道：" + menmsg[random(sizeof(menmsg))];
    ...
    switch (n) {
        case 1:
            return cob->name(1) + (random(2) ? "：怎么，没人敢应战么？那我就再等等！"
                : "：嘿嘿，都是一些胆小鬼！");

        case 3:
            return cob->name(1) + (random(2) ? "似乎等得很不耐烦。"
                : "一会儿看看天空，一会儿望望地面，看起来很是悠闲。");

        case 5:
            return cob->name(1) + (random(2) ? "：怎么，还有没人敢应战么？真是让我失望！"
                : "：说什么中土武学博大精深，原来都是谣传。");

        case 7:
            return cob->name(1) + (random(2) ? "摇摇头，又叹叹气，一副不屑的样子。”"
                : "哼了一声，满脸尽是不屑。");

        case 9:
            return cob->name(1) + (random(2) ? "：算了！我看我还是走吧！"
                : "：我也算是来过了，看来还是不要在这浪费时间了。");

        case 10:
            return cob->name(1) + (random(2) ? "：中原武林，全都是一些缩头乌龟！算了，我走了！"
        ...
        case 11:
            CHANNEL_D->do_channel(
                this_object(),
                "rumor",
                "听说来自" + cob->query("nation") + "的" + cob->name(1) + "在中国耀武扬威了一番后安然的回国去了，真是耻辱。"
            );
            return 0;
    }
...
string cannot_accept(object ob) {
    ...
    if (ob->query("combat_exp") < cob->query("combat_exp") / 2)
        return "你的武功和挑战者相比差的太远，还是不要丢我们中国人的脸了！\n";
    ;
...
void accept_for_challenge(object me) {
    ...
    CHANNEL_D->do_channel(
        this_object(),
        "rumor",
        "听说" + fam + me->name(1) + "挺身而出，接下了" + cob->name(1) + "的挑战！"
    );
    rob = me;
    accepted = me->name(1);
