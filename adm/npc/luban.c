...
private int create_new_key(object me) {
    ...
    content = replace_string(
        content,
        "这是原配钥匙",
        "这是" + me->query("name") + "第" + chinese_number(n) + "次配的钥匙"
    );
...
int demolish_room(object me) {
    ...
    room_name = me->query("private_room/name") + "(" + me->query("private_room/id") + ")";
...
private void show_desc(mixed player, string room_name) {
    ...
    if (mapp(query("form/" + player_id))) {
        ...
        room_nameid = player->query_temp("contract/luban/name") + "(" + player->query_temp("contract/luban/id") + ")";
...
private void promote_type(object me) {
    ...
    msg = "你眼光略略一扫，看到的房屋类型有：" + room_example[0]["name"] + "(" + room_example[0]["type"] + ")";
    len = strlen(msg);
    for (i = 1; i < sizeof(room_example); i++) {
        string temp;

        temp = "、" + room_example[i]["name"] + "(" + room_example[i]["type"] + ")";
        msg += temp;
        len += strlen(temp);
        if (len > 70) {
            msg += "\n";
...
private int record_desc(object me, string room_name, string desc) {
    ...
    for (i = 0; i < sizeof(names); i++) {
        ...
        if (ob->query("short") == room_name || names[i] == room_name) {
            ...
            if (!desc || desc == "") {
                string msg;
                msg = "你没有输入描述" + ob->query("short") + "(" + room_name + ")的信息，将使用系统缺省的描述信息。\n";
                msg += CYN "缺省的描述信息如下：\n";
                if (stringp(ob->query("default_long")))
