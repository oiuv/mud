...
void setup() {
    set("item_desc/river", HIC "\n只见近岸处有一叶小舟，也许大声喊(" HIY "yell" HIC ")一声船家(" HIY "boat" HIC
        ")就\n能听见。倘若你自负轻功绝佳,也可直接"
...
int do_yell(string arg) {
    ...
    msg = WHT "\n$N" NOR + WHT "鼓足中气，高喊一声：「" + arg + "」声音远远传了出去。\n" NOR;

    if (arg != "船家" || !arrive) {
        msg += HIY "突然江面上远远传来一阵回声：「" + arg + "~~~~~~」\n" NOR;
        ...
        obs = filter_array(all_inventory(),
            (: $1 == $(me) || $1->query_leader() == $(me) && !playerp($1) && !$1->is_killing($(me)->query("id")) :));
        obs->move(boat);

        me->start_call_out((: call_other, __FILE__, "arrival", me, boat, arrive :), 3);
    }
    return 1;
}
...
void arrival(object me, object boat, object arrive) {
    ...
    obs = filter_array(all_inventory(boat),
        (: $1 == $(me) || $1->query_leader() == $(me) && !playerp($1) && !$1->is_killing($(me)->query("id")) :));
