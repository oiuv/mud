...
mapping mixed_poison(mapping p1, mapping p2) {
    ...
    // generate the new id
    if (!p1["id"]) p1["id"] = p2["id"]; else if (!p2["id"]) p2["id"] = p1["id"];
    ...
    if (!p1["name"]) p1["name"] = p2["name"]; else if (!p2["name"]) p2["name"] = p1["name"];
...
int dispel(object me, object ob, mapping cnd) {
    ...
    my_lvl = me->query_skill("force") + me->query_skill("poison") / 5 + me->query_skill(
        "dispel-poison",
        1
    ) + me->query_skill("medical") / 5 + me->query_temp("apply/dispel-poison");
    if (need_lvl > my_lvl) {
        if (me == ob) {
    ...
    if (me == ob) {
        ...
        if (cnd["id"] == me->query("id")) {
            ...
            if (dis >= cnd["remain"]) {
                dis = cnd["remain"];
                cost_neili = cnd["remain"] * 4 * (cnd["level"] + 1) / power;
                tell_object(me, WHT "你运用内功，帮助" + ob->name() +
                    "将" + pos + "尽数驱散。\n" NOR);
            } else {
                tell_object(me, WHT "你运用内功，帮助" + ob->name() +
