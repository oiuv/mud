...
mixed ask_jian() {
    ...
    if (me->is_bad()) {
        if (me->query("family/family_name") == "红花会")
            message_vision(CYN "$N" CYN "冷哼一声，对$n" CYN "道："
                ...
                "花会清除你这个败类！”\n" NOR,
                this_object(), me);
        else if (me->query("family/family_name") == "武当派")
            message_vision(CYN "$N" CYN "冷哼一声，对$n" CYN "道："
                "“你身在武当，行为却如此不耻，竟还敢在"
                "我面前说三道四！”\n" NOR,
                this_object(), me);
    ...
    if (objectp(owner) && owner != this_object()) {
        ...
        if (owner->query("family/family_name") == "武当派")
            return "我的配剑现在在我武当门人" + owner->query("name") + "手中，你要用就去找他吧。";
        else if (owner->query("family/family_name") == "红花会")
            return "我的配剑被你们红花会的" + owner->query("name") + "取去了，你要用就去找他吧。";
        else
            return "我的配剑现在落到了" + owner->query("name") + "手中，你去找他吧！";
    }
    ob->move(this_object());
