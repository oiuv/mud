...
mixed ask_skill3()
{
    ...
    if (me->query("can_perform/guanri-jian/guan"))
        return "自己去领悟吧，好自为之。";

    if (me->query("family/family_name") != query("family/family_name"))
        return "给我滚开。";

    if (me->query("family/master_id") != "ma dayuan" && me->query("family/master_id") != "hong qigong" && me->query("family/beggarlvl") < 7)
        return "我什么时候说过要教你？";
    ...
    if (me->query_skill("guanri-jian", 1) < 280)
        return "你的观日剑法还不到家，要多练练。";
    ...
    me->improve_skill("martial-cognize", 1500000);
    me->set("can_perform/guanri-jian/guan", 1);
