...
mixed ask_book1() {
    ...
    if (objectp(owner) && owner != this_object()) {
        ...
        if (owner->query("family/family_name") == "桃花岛")
            return "那本「河图」现在在我师兄" + owner->query("name") + "手中，你要看就去找他吧。";
        else
            return "那本「河图」现在已落到了" + owner->query("name") + "手中，你去找他吧！";
    }
    ob->move(this_object());
...
mixed ask_book2() {
    ...
    if (objectp(owner) && owner != this_object()) {
        ...
        if (owner->query("family/family_name") == "桃花岛")
            return "那本「洛书」现在在我师兄" + owner->query("name") + "手中，你要看就去找他吧。";
        else
            return "那本「洛书」现在已落到了" + owner->query("name") + "手中，你去找他吧！";
    }
    ob->move(this_object());
