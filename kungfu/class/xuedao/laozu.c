...
mixed ask_dao() {
    ...
    if (objectp(owner) && owner != this_object()) {
        ...
        if (owner->query("family/family_name") == "血刀门")
            return "血刀现在暂时是你同门师兄" + owner->query("name") + "在用，你要用就去找他吧。";
        else
            return "本门的血刀现在落在" + owner->query("name") + "手中，你去把他做了，把血刀抢回来！";
    }
...
mixed ask_book() {
    ...
    if (objectp(owner) && owner != this_object()) {
        ...
        if (owner->query("family/family_name") == "血刀门")
            return "血刀经现在暂时被你同门的" + owner->query("name") + "取去看了，你要看就去找他吧。";
        else
            return "我血刀门的宝经现在落在了" + owner->query("name") + "手中，你去把它夺回来吧！";
    }
    ob->move(this_object());
