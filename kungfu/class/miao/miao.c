...
mixed ask_jian() {
    ...
    if (objectp(owner) && owner != this_object()) {
        ...
        if (owner->query("family/family_name") == "中原苗家")
            return "腾龙剑现在暂时是你兄弟" + owner->query("name") + "在用，你要用就去找他吧。";
        else
            return "我的腾龙剑现在落在了" + owner->query("name") + "手中，你去把它找回来吧！";
    }
    ob->move(this_object());
...
mixed ask_jianpu1() {
    ...
    if (objectp(owner) && owner != this_object()) {
        ...
        if (owner->query("family/family_name") == "中原苗家")
            return "苗家剑谱现在暂时被你兄弟" + owner->query("name") + "取去看了，你要看就去找他吧。";
        else
            return "我们苗家的家传剑谱现在落在了" + owner->query("name") + "手中，你去把它夺回来吧！";
    }
    ob->move(this_object());
