...
void do_continue_help(object ob)
{
    ...
    message("vision", HIR "说时迟，那时快！突然转出" + chinese_number(n) + "个人，一起冲上前来，看来是早"
            "有防备！\n" NOR, environment());
        ...
        if (random(100) < 8)
        {
            ...
            case 4:
                msg = HIC "$n" HIC "连连进击，眼看"
                          "便要得手，接连数招，让$N" HIC
                          "已是避无可避，退无可退！$N" HIC "神色神色仓皇，急切间猛地"
                          "咬中舌尖，“噗”的吐出！$n" HIC "只见眼前一阵红雾，待得雾"
                          "散去，哪里还有$N" HIC "的影"
                          "子？\n" NOR;
                break;
            case 5:
            ...
            default:
                msg = HIC "$N身负重伤，已然不敌，悲"
                          "愤间只放声疾呼，一时震得$n" HIC "耳鼓欲破，攻势少缓。$N" HIC "猛然见到有机可乘，哪里肯"
                          "放过，连出数招，杀出一条血路"
                          "，就此不见。\n" NOR;
                break;
            }
...
void die(object killer)
{
    ...
    while (1)
    {
        ...
        if (dob)
            aob = dob->query_temp("quest/assist");
    ...
    if ((n = sizeof(obs)) > 0)
    {
        ...
        if (objectp(dob) && member_array(dob, obs) != -1)
        {
            ...
            else if (dob->query("combat_exp") > query("combat_exp") * 2)
            {
                // 出手的人武功太高，获得的额外奖励少
                extra_exp /= 2;
                extra_pot /= 2;
        ...
        foreach (hob in obs)
        {
            ...
            // 看到奖励。
            GIFT_D->delay_bonus(hob, ([ "exp":exp + random(4) + ((hob == dob) ? extra_exp : 0),
                                        "pot":pot + random(3) + ((hob == dob) ? extra_pot : 0),
