...
int valid_learn(object me)
{
        if (me->query_temp("weapon") || me->query_temp("secondary_weapon"))
                return notify_fail("练缥缈掌法必须空手。\n");

        if ((int)me->query_skill("force") < 30)
                return notify_fail("你的内功火候不够，无法学缥缈掌法。\n");

        if ((int)me->query("max_neili") < 100)
                return notify_fail("你的内力太弱，无法练缥缈掌法。\n");
        ...
        if ((int)me->query_skill("strike", 1) < (int)me->query_skill("piaomiao-zhang", 1))
                return notify_fail("你的基本掌法水平有限，无法领会更高深的缥缈掌法。\n");
...
int practice_skill(object me)
{
        ...
        if ((int)me->query("neili") < 50)
                return notify_fail("你的内力不够练缥缈掌法。\n");
