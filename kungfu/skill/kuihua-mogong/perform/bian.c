...
int perform(object me, object target) {
    ...
    msg = HIR "$N" HIR "一声尖啸，身体猛然旋转不定，" + name + HIR "顿时化成无数气流，犹如千万根银针，齐齐卷向$n" HIR "！\n" NOR;

    ap = me->query_skill("kuihua-mogong", 1) + me->query_skill("dodge");

    dp = target->query_skill("martial-cognize", 1) + target->query_skill("dodge");
