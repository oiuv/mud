...
int valid_learn(object me)
{
        ...
        if ((int)me->query("max_neili") < 100)
                return notify_fail("你的内力太弱，无法练华山拳法。\n");

        if ((int)me->query_skill("cuff", 1) < 20)
                return notify_fail("你的基本拳法火候太浅。\n");

        if ((int)me->query_skill("cuff", 1) < (int)me->query_skill("huashan-quanfa", 1))
...
{
        int i, level;
        level = (int) me->query_skill("huashan-quanfa", 1);
...
{
        return __DIR__"huashan-quanfa/" + action;
}
