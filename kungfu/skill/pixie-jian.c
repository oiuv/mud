...
mapping *action = ({
    ...
    ([ "action": "$N凝神静气，使出极招" RED " 辟邪剑之极意 " NOR "",
...
int valid_enable(string usage) {
    return usage == "sword" || usage == "dodge" || usage == "parry" || usage == "unarmed";
}
...
mixed valid_damage(object ob, object me, int damage, object weapon) {
    ...
    dp = me->query_skill("dodge", 1) / 2 + me->query_skill("pixie-jian", 1);
...
mixed hit_ob(object me, object victim, int damage_bonus) {
    if (damage_bonus > 80 && random(10) >= 5 && (me->query("special_skill/guimai") || me->query_skill(
        "pixie-jian",
        1
    ) >= 200)) {
        if (random(3)) {
            victim->receive_wound("qi", (damage_bonus - 80) / 4, me);
            return HIR "只见" + victim->name() + HIR "不寒而栗，身上数处地方渗出血丝！\n" NOR;
        } else {
            victim->receive_wound("qi", (damage_bonus - 80) / 5, me);
            victim->receive_wound("jing", (damage_bonus - 80) / 4, me);
            return HIR "只见" + victim->name() + HIR "神情恍惚，鲜血从身上数个穴位渗出！\n" NOR;
        }
    }
}
