...
mixed valid_damage(object ob, object me, int damage, object weapon) {
    ...
    if ((int)me->query_skill(
        "taixuan-gong",
        1
    ) < 150 || me->query_skill_mapped("parry") != "taixuan-gong" || me->query_skill_mapped("force") != "taixuan-gong" || me->query_skill_mapped("unarmed") != "taixuan-gong" || (int)me->query("neili") < 500 || ob->query_temp("weapon") || !living(me))
        return;
...
mixed hit_ob(object me, object victim, int damage_bonus, int factor) {
    ...
    if (damage_bonus < 150 || lvl < 140 || me->query("neili") < 300 || me->query_skill_mapped("force") != "taixuan-gong")
        return 0;
    ...
    if (me->query("can_learned/taixuan-gong/enable_weapon")) {
        ...
        if (objectp(weapon) && weapon->query("skill_type") != "sword" && weapon->query("skill_type") != "blade")
            return 0;

        if (objectp(weapon) && weapon->query("skill_type") == "sword" && me->query_skill_mapped("sword") != "taixuan-gong")
            return 0;

        if (objectp(weapon) && weapon->query("skill_type") == "blade" && me->query_skill_mapped("blade") != "taixuan-gong")
            return 0;
    }
...
int valid_learn(object me) {
    ...
    if ((int)me->query_skill("literate", 1) || me->query("learned_literate"))
