...
mapping *action = ({
    ...
    ([ "action": "$N凝神静气，使出极招" RED " 太极剑之极意 " NOR "",
        "force": (int)this_player()->query_skill(
            "force",
            1
        ) / 4 + random((int)this_player()->query_skill("force", 1) / 3),
        "attack": (int)this_player()->query_skill(
            "sword",
            1
        ) / 5 + random((int)this_player()->query_skill("sword", 1) / 3),
        "dodge": (int)this_player()->query_skill(
            "dodge",
            1
        ) / 4 + random((int)this_player()->query_skill("force", 1) / 2),
        "parry": (int)this_player()->query_skill(
            "parry",
            1
        ) / 3 + random((int)this_player()->query_skill("parry", 1)),
        "damage": (int)this_player()->query_skill(
            "force",
            1
        ) / 4 + random((int)this_player()->query_skill("sword", 1) / 4),
        "lvl": 350,
        "skill_name": "极意",
        "damage_type": "刺伤"
    ]),
...
mixed valid_damage(object ob, object me, int damage, object weapon) {
    ...
    dp = me->query_skill("parry", 1) / 2 + me->query_skill("taiji-jian", 1);
    ...
    if (ap / 2 + random(ap) < dp) {
        ...
        switch (random(2)) {
            case 0:
                result = HIY "$n" HIY "一抖手中的" + m_weapon->name() + HIY "，划出了一个圆圈，$N"
            ...
            default:
                result = HIY "$n" HIY "晃动手中的" + m_weapon->name() + HIY "，不住的变幻剑势，然而$N"
