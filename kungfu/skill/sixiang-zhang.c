...
int valid_learn(object me)
{
        if (me->query_temp("weapon") || me->query_temp("secondary_weapon"))
                return notify_fail("练四象掌法必须空手。\n");

        if (me->query_skill("force") < 20)
                return notify_fail("你的内功火候不够，不能练四象掌法。\n");

        if (me->query("max_neili") < 50)
                return notify_fail("你的内力太弱，无法练四象掌法。\n");

        if (me->query_skill("strike", 1) < me->query_skill("sixiang-zhang", 1))
                return notify_fail("你的基本掌法太差，无法领会更高深的四象掌法。\n");
...
int practice_skill(object me)
{
        ...
        if ((int)me->query("neili") < 60)
                return notify_fail("你的内力不够练习四象掌法。\n");
