...
int perform(object me, object target) {
    ...
    ap = me->query_int();
    dp = target->query_int();
    msg += "\n" HIC "$N" HIC "随即一声冷笑，手中的" + wname + HIC "伸伸缩缩，吞吞吐吐，变幻莫测，层层剑光笼罩$n" HIC "。\n" NOR;
    if (ap / 2 + random(ap) > dp) {
        msg += COMBAT_D->do_damage(me, target, WEAPON_ATTACK, damage, 10,
    ...
    ap = me->query_con();
    dp = target->query_con();
    msg += "\n" HIC "$N" HIC "深深吸了一口气，手中的" + wname + HIC "幻化出一层浅浅的光芒，人剑合一扑向$n" HIC "而去。\n" NOR;
    if (ap / 2 + random(ap) > dp) {
    ...
    ap = me->query_dex();
    dp = target->query_dex();
    msg += "\n" HIC "$N" HIC "脚步一晃，忽然近上身来，身法奇快，手中的" + wname + HIC "应声而起，刺向$n" HIC "。\n" NOR;
    if (ap / 2 + random(ap) > dp) {
    ...
    msg += "\n" HIC "$N" HIC "一声长叹，眼神变得寒冷无情，" + wname + HIC "悠悠而转，犹如轻风拂柳，说不清的缠绵之意。\n" NOR;
    if (ap / 2 + random(ap) > dp) {
        msg += COMBAT_D->do_damage(me, target, WEAPON_ATTACK, damage, 20,
