...
int valid_force(string force) {
    return force == "mizong-neigong" || force == "xiaowuxiang" || force == "xuedao-dafa";
}
...
mixed hit_ob(object me, object victim, int damage_bonus, int factor) {
    ...
    if (damage_bonus < 130 || layer < 6 || me->query("neili") < 500 || me->query_temp("weapon") || me->query_temp("secondary_weapon") || me->query_skill_mapped("force") != "longxiang-gong" || me->query_skill_mapped("unarmed") != "longxiang-gong" || me->query_skill_prepared("unarmed") != "longxiang-gong" || damage_bonus / 6 < victim->query_con() || random(10) < 5)
        return 0;

    if (layer >= 10 && random(2) == 1) {
        me->add("neili", -80);
        victim->receive_wound("qi", (damage_bonus - 50) / 3, me);
        return HIY "$N" HIY "拳锋之上竟蕴藏着『" HIR "十龙十象" HIY "』之力，罡劲破体，摧毁了$n" HIY "的真元！\n" NOR;
    } else {
        me->add("neili", -50);
        victim->receive_wound("qi", (damage_bonus - 100) / 3, me);
        return HIY "$N" HIY "双拳暗含龙象般若功第" + chinese_number(layer) + "层功力，$n" HIY "心脉受震，喷出一口鲜血！\n" NOR;
    }
}
