 ...
 */
    ...
    msg = HIG "$N" HIG "手中扣住一枚" + weapon->name() + HIG "，不理敌人的攻击，身体急速旋转起来，犹如一道呼啸的旋风！\n" NOR HIG "突然从旋风中飞出无数点" HIC "寒星" NOR + HIG "，划出一道道长虹闪电般的飞向$n" HIG "的身体！\n" NOR;

    ap = me->query_skill("throwing") + me->query_skill("force");
    dp = target->query_skill("parry") + target->query_skill("dodge") + target->query_skill(
        "dugu-jiujian",
        1
    );
    ...
    if (ap / 2 + random(ap) > dp) {
        if (weapon->query("id") != "sanhua tianyu") {
            ...
            msg = HIR "结果$p" HIR "一声惨嚎，连中了$P" HIR "发出的十" + weapon->query("base_unit") + weapon->name() + HIR "。\n" NOR;
