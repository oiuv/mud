...
int exert(object me, object target) {
    ...
    msg = HIR "\n$N" HIR "伸出食指，点在" + target->name() + HIR "腹部，默运内力迫出毒液练药。\n" NOR;

    p = target->query("worm_poison");
    if (!mapp(p))
        return notify_fail("看来你是弄不出什么毒液来了。\n" NOR);

    lvl = (int)me->query_skill(
        "poison",
        1
    ) / 3 + (int)me->query_skill(
        "wudu-qishu",
        1
    ) / 3 + (int)me->query_skill("xiuluo-yinshagong", 1) / 3;

    amount = p["level"] * p["remain"];
    if (amount > lvl)
        amount = lvl;

    p["remain"] = (p["level"] * p["remain"] - amount) / p["level"];
