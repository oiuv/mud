...
int perform(object me, object target) {
    ...
    msg = HIW "$N" HIW "右手一扬，手中" + weapon->name() + HIW "顿时回旋飞舞而至，迂回盘绕向$n" HIW "！\n";
    ...
    if (random((int)me->query_skill(
        "jiandun-zhusuo",
        1
    )) > (int)target->query_skill("parry", 1) / 2) {
        msg += HIR "$n" HIR "只觉全身一紧，已被" + weapon->name() + HIR "牢牢缚住，动弹不得！\n" NOR;
