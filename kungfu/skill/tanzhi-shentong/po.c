...
int perform(object me, object target) {
    ...
    msg = HIW "陡见$N" HIW "双目精光四射，顿听破空声大作，一" + weapon->query("base_unit") + weapon->name() + HIW "由"
        "指尖弹出，疾速射向$n" HIW "。\n" NOR;
    ...
    if (ap / 2 + random(ap) > dp) {
        ...
        msg += HIR "只见那" + weapon->query("base_unit") + weapon->name() + HIR "来势迅猛之极，$n" HIR
            "根本无暇闪避，被这招击个正中！\n" NOR;
        ...
        if ((weapon2 = target->query_temp("weapon"))
            && ap / 3 + random(ap) > dp) {
            msg += HIW "$n" HIW "手腕一麻，手中" + weapon2->name() + HIW "不由脱手而出！\n" NOR;
