...
mixed valid_damage(object ob, object me, int damage, object weapon) {
    ...
    if ((int)me->query_skill(
        "jiuyin-shengong",
        1
    ) < 150 || me->query_skill_mapped("parry") != "jiuyin-shengong" || me->query_skill_mapped("force") != "jiuyin-shengong" || (int)me->query("neili") < 500 || !living(me) || ob->query_temp("weapon") || random(2) != 1)
        return;
...
mixed hit_ob(object me, object victim, int damage_bonus, int factor) {
    ...
    if (me->is_busy() || random(2) == 0 || !living(victim) || damage_bonus < 120 || me->query("neili") < 200 || me->query_skill(
        "jiuyin-shengong",
        1
    ) < 250)
        return 0;
