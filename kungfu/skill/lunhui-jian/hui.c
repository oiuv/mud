...
int perform(object me, object target) {
    ...
    ap = me->query_skill("sword") + me->query_skill("buddhism", 1);

    dp = target->query_skill("parry") + target->query_skill("buddhism", 1);
...
string attack2(object me, object target, int damage) {
    ...
    if (objectp(weapon = target->query_temp("weapon"))) {
        wn = weapon->name();
        msg += WHT "忽听「锵锵锵」几声脆响，$n" WHT "的" + wn + WHT "竟被$N" WHT "绞成了块块碎片。\n" NOR;
...
string attack4(object me, object target, int damage) {
    ...
    if (objectp(cloth = target->query_temp("armor/cloth"))) {
        cn = cloth->name();
        msg += WHT "忽听轰然声大作，$n" WHT "身着的" + cn + WHT "在$N" WHT "内力激荡下，竟被震得粉碎。\n"
            NOR;

        cloth->set("consistence", 0);
        cloth->move(target);
    } else if (objectp(armor = target->query_temp("armor/armor"))) {
        an = armor->name();
        msg += WHT "忽听「哧啦」一声脆响，$n" WHT "身着的" + an + WHT "竟被$N" WHT "震裂，化成块块碎片。\n"
            NOR;
