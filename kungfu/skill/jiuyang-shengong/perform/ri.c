...
int perform(object me) {
    ...
    ap = me->query_skill(
        ...
        "martial-cognize",
        1
    ) + me->query_skill("jiuyang-shengong", 1) + me->query("con") * 10;
    ...
    for (flag = 0, i = 0; i < sizeof(obs); i++) {
        ...
        if (ap / 2 + random(ap) > dp) {
            ...
            switch (random(5)) {
                ...
                case 2:
                    msg = HIR "只见" + obs[i]->name() + HIR "跌跌撞撞向后连退数步，伏倒"
                        "在地。须眉、衣衫都发出一股焦臭"
                        "。\n" NOR;
                    msg += "( " + obs[i]->name() + eff_status_msg(p) + " )\n\n";
                    break;
                ...
                case 3:
                    msg = HIR "光芒闪过，" + obs[i]->name() + HIR "却是呆立当场，动也不动，七"
                        "窍流血，神情扭曲，煞是恐怖。\n" NOR;
                    msg += "( " + obs[i]->name() + eff_status_msg(p) + " )\n\n";
                    break;

                default:
                    msg = HIR + obs[i]->name() + HIR "急忙抽身后退，可只见眼前光"
                        "芒暴涨，一闪而过。全身已多了数"
                        "个伤口，鲜血飞溅。\n" NOR;
                    msg += "( " + obs[i]->name() + eff_status_msg(p) + " )\n\n";
                    break;
