...
mixed valid_damage(object ob, object me, int damage, object weapon) {
    ...
    dp = me->query_skill("parry", 1) / 2 + me->query_skill("dugu-jiujian", 1);
    ...
    if (ap / 2 + random(ap) < dp) {
        ...
        switch (random(3)) {
            case 0:
                result = HIY "$n" HIY "一抖手中的" + m_weapon->name() + HIY "，中攻直进一剑挺进，可是$N" HIY "反招更快"
                    "，进袭$n" HIY "！\n" NOR;
                break;
    ...
    switch (random(3)) {
        case 1:
            if (ap * 3 / 4 + random(ap) > dpp) {
                ...
                return random(2) ? HIR "$N" HIR "一剑攻出，剑气横飞，" + weapon->name() + HIR
                    ...
                    "一剑之中竟蕴藏着如此威力。\n" NOR : HIR "$N" HIR "手中" + weapon->name() + HIR "犹"
        ...
        case 2:
            ...
            if (!me->is_busy()) {
                ...
                msg = HIC "$N" HIC "随意挥洒着手中的" + weapon->name() + HIC "刺向$n" HIC "，不过$n" HIC "防守的异常严密，$N"
                    HIC "一时竟然无法找到破绽。\n" NOR;
