...
int perform(object me, object target) {
    ...
    if (weapon)
        msg = HIC "\n$N" HIC "舞动手中的" + weapon->name() + HIC "，朝着" + target->name() + HIC "的七寸打"
            "了过去。\n" NOR;
    else
    ...
    lvl = (int)me->query_skill("shedu-qiqiao", 1) + (int)me->query_skill("dodge");
    lvl = lvl * lvl / 10 * lvl;

    if (lvl / 2 + random(lvl) > (int)target->query("combat_exp")) {
        msg += HIY "结果只听“啪”的一声，正打在" + target->name() + HIY "的七寸上。\n" NOR;
        ...
        if (lvl / 2 + random(lvl) > (int)target->query("combat_exp")) {
            ...
            msg += HIR + "哪里想到" + target->name() + HIR "挨了这一击，竟然若无其事，顿时一个翻"
                "身，直扑向$N" HIR "！\n\n" NOR;
            message_vision(msg, me);
            target->kill_ob(me);
        }
