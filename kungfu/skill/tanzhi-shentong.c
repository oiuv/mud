...
mapping *action_finger = ({
    ...
    ([ "action": "$N凝神静气，使出极招" RED " 弹指神通之极意 " NOR "",
        ...
        "attack": (int)this_player()->query_skill(
            "finger",
            1
        ) / 4 + random((int)this_player()->query_skill("finger", 1) / 2),
...
int valid_enable(string usage) {
    return usage == "finger" || usage == "parry" || usage == "throwing";
}
...
mixed valid_damage(object ob, object me, int damage, object weapon) {
    ...
    dp = me->query_skill("parry", 1) / 2 + me->query_skill("tanzhi-shentong", 1);
...
mixed hit_ob(object me, object victim, int damage_bonus) {
    ...
    return HIR "只听“飕”的一声，$n" HIR "被$N" HIR "凌空弹射出的指气点中" NOR + HIY + name + NOR + HIR "，全身登时一阵酸麻。\n" NOR;
}
