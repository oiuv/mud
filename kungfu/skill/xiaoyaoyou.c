...
int valid_enable(string usage) {
    return usage == "dodge" || usage == "parry" || usage == "unarmed";
}
...
int valid_learn(object me) {
    ...
    if ((int)me->query_skill("force") < 100)
        return notify_fail("你的内功火候不够，无法学逍遥游拳法。\n");

    if ((int)me->query("max_neili") < 800)
        return notify_fail("你的内力修为太弱，无法练逍遥游拳法。\n");

    if ((int)me->query_skill("unarmed", 1) < (int)me->query_skill("xiaoyaoyou", 1))
        return notify_fail("你的基本拳脚水平有限，无法领会更高深的逍遥游拳法。\n");

    if ((int)me->query_skill("dodge", 1) < (int)me->query_skill("xiaoyaoyou", 1))
        return notify_fail("你的基本轻功水平有限，无法领会更高深的逍遥游拳法。\n");
