...
int exert(object me, object target) {
    ...
    msg = HIM "$N" HIM "一声冷笑，默运" + to_chinese(f) + HIM "内劲，手指粘住" + du->name() + HIM "对准$n" HIM "「嗖」的弹射了出去。\n" NOR;
    ...
    if (an / 2 + random(an) < dn * 2 / 3) {
        ...
        ap = me->query_skill("force") + me->query_skill("poison") + me->query_skill("throwing");

        dp = target->query_skill("dodge") + target->query_skill("parry") + target->query_skill(
            "martial-cognize",
            1
        );
