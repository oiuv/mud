...
mapping *action = ({
    ...
    ([ "action": "$N凝神静气，使出极招" RED " 太极拳之极意 " NOR "",
        ...
        "attack": (int)this_player()->query_skill(
            "unarmed",
            1
        ) / 5 + random((int)this_player()->query_skill("unarmed", 1) / 3),
        ...
        "damage": (int)this_player()->query_skill(
            "force",
            1
        ) / 4 + random((int)this_player()->query_skill("unarmed", 1) / 4),
        "lvl": 350,
        "skill_name": "极意",
        "damage_type": "瘀伤"
    ]),
...
mixed valid_damage(object ob, object me, int damage, object weapon) {
    ...
    dp = me->query_skill("parry", 1) / 2 + me->query_skill("taiji-quan", 1);
