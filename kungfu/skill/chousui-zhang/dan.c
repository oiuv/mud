...
int perform(object me, object target) {
    ...
    } else if (lvl >= 180 && lvp >= 240) {
        pos = 200 + random(lvp * 3 / 4);
        fire = "一个殷红色的火球";
    } else if (lvl >= 150 && lvp >= 210) {
...
string final(object me, object target, int damage) {
    ...
    if (objectp(cloth = target->query_temp("armor/cloth"))) {
        ...
        name = cloth->name();
    } else if (objectp(cloth = target->query_temp("armor/armor"))) {
        if (cloth->query("stable", 1) < 100) {
            cloth->add("consistence", -1 - random(10));
    ...
    return HIR "$n" HIR "一个不慎，火星顿时溅到" + name + HIR "之上，大势燃烧起来，皮肉烧得嗤嗤作响。\n" NOR;
}
