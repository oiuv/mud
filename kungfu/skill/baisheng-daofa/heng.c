...
int perform(object me, object target) {
    ...
    if (me->query("character") == "光明磊落") {
        msg = HIR "$N" HIR "嗔目大喝，杀气凛然而起，手持" + weapon->name() + HIR "义无反顾的猛然横扫$n" HIR "，全然不顾生死，浩气怅然！\n" NOR;
        ap += ap * 1 / 4;
    } else
        msg = HIR "$N" HIR "狂提内劲，将全身力道灌注于" + weapon->name() + HIR "之上疾斩劈出，“唰”的一声扫向$n" HIR "而去！\n" NOR;
...
string final(object me, object target, int ap, int dp) {
    ...
    } else if ((ap / 2 > dp) && objectp(weapon = target->query_temp("weapon"))) {
