...
int valid_force(string force) {
    return force == "quanzhen-xinfa" || force == "duanshi-xinfa" || force == "suxin-jue" || force == "yunv-xinjing" || force == "kurong-changong" || force == "xuanmen-neigong";
}
...
int valid_learn(object me) {
    ...
    if (me->query_skill(
        "taoism",
        1
    ) < 320 && me->query_skill("taoism", 1) < me->query_skill("xiantian-gong", 1))
...
mixed hit_ob(object me, object victim, int damage_bonus, int factor) {
    ...
    if (damage_bonus < 150 || lvl < 180 || me->query("neili") < 300 || me->query_temp("weapon") || me->query_temp("secondary_weapon") || me->query_skill_mapped("force") != "xiantian-gong" || me->query_skill_mapped("unarmed") != "xiantian-gong" || me->query_skill_prepared("unarmed") != "xiantian-gong")
        return 0;

    if (damage_bonus / 6 > victim->query_con()) {
        me->add("neili", -30);
        victim->receive_wound("qi", (damage_bonus - 100) / 3, me);
        return HIW "$N" HIW "先天功的潜力发挥了出来，突然间$n" HIW "只感喉咙一甜，喷出一口鲜血！\n" NOR;
    }
}
