...
int perform(object me, object target) {
    ...
    // 第一招，判断对方臂力
    dp = target->query_str() * 2 + target->query_skill(
        "unarmed",
        1
    ...
    // 第二招，判断对方悟性
    dp = target->query_int() * 2 + target->query_skill(
        "dodge",
        1
    ) + target->query_skill("parry", 1);
    ...
    // 第三招，判断对方根骨
    dp = target->query_con() * 2 + target->query_skill(
        "force",
        1
    ) + target->query_skill("parry", 1);
    ...
    // 第四招，判断对方身法
    dp = target->query_dex() * 2 + target->query_skill(
        "dodge",
        1
    ) + target->query_skill("parry", 1);
