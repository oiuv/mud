...
int perform(object me) {
    ...
    ap = me->query_skill(
        ...
        "martial-cognize",
        1
    ) + me->query_skill("taixuan-gong", 1) + me->query("con") * 10;

    me->add("neili", -(sizeof(obs) * 220));

    for (flag = 0, i = 0; i < sizeof(obs); i++) {
        dp = obs[i]->query_skill("force") * 2 + obs[i]->query_skill(
            "martial-cognize",
            1
        ) + obs[i]->query("con") * 10;
        ...
        if (ap * 2 / 3 + random(ap) > dp) {
            ...
            switch (random(5)) {
                case 0:
                    msg = HIR "只听" + obs[i]->name() + HIR "一声惨嚎，接连退了数步，“"
                        "哇”的呕出一大口鲜血。\n" NOR;
                    msg += "( " + obs[i]->name() + eff_status_msg(p) + " )\n\n";
                    break;

                case 1:
                    msg = HIR "只见" + obs[i]->name() + HIR "向后飞出丈远，重重的跌落在"
                        "地上，衣衫破烂，再也无法站起来"
                        "。\n" NOR;
                    msg += "( " + obs[i]->name() + eff_status_msg(p) + " )\n\n";
                    break;
                ...
                case 2:
                    msg = HIR "只见" + obs[i]->name() + HIR "歪歪斜斜倒退几步，伏倒"
                        "在地，痛苦不堪。"
                        "。\n" NOR;
                    msg += "( " + obs[i]->name() + eff_status_msg(p) + " )\n\n";
                    break;
                ...
                case 3:
                    msg = HIR "狂风卷过，" + obs[i]->name() + HIR "只见，飞沙狂舞，却动也动不了"
                        "忽然间，却瘫软在地。\n" NOR;
                    msg += "( " + obs[i]->name() + eff_status_msg(p) + " )\n\n";
                    break;

                default:
                    msg = HIR + obs[i]->name() + HIR "急忙飞身而起，却猛然坠地，伤痕遍体，鲜"
                        "血不止。\n" NOR;
                    msg += "( " + obs[i]->name() + eff_status_msg(p) + " )\n\n";
                    break;
            }
