...
mixed hit_ob(object me, object victim, int damage_bonus, int factor) {
    ...
    if (random(me->query_skill("shenfeng-bian", 1)) > 60
        ...
        if (victim->query_temp("weapon")) {
            if (random(5) > 2) {
                message_vision(
                    HIY "只见$N急速挥舞" + weap1->name() + HIY "，转出无数个大小圈子，以阴柔之劲缠住对方的" + weap->name() + "！\n" NOR,
                    me
                );
                victim->start_busy(1);
            ...
            if ((random(me->query_skill("parry", 1)) > victim->query_skill("parry", 1) / 3)
                ...
                victim->receive_damage(
                    "qi",
                    me->query_skill("kurong-changong", 1) / 2 + damage_bonus / 2
                );
                ...
                message_vision(
                    HIY "只见$N的鞭式灵动异常," + weap1->name() + HIY "转出无数个大小圈子，以阴柔之劲缠住了对方的" + name + "！\n" NOR,
                    me
                );
