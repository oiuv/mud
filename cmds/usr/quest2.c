    ...
    for (i = 0; i < size; i++) {
        ...
        if (me->getKilled(quest_file, key[i]) >= value[i])
            msg += sprintf(
                ...
                HIK + ob->name(),
                HIK + ob->query("id"),
                HIK + me->getKilled(quest_file, key[i]) + "/" + value[i]
            );
        else
            msg += sprintf(
                "  %s(%s)： %d/%d\n",
    ...
    for (i = 0; i < size; i++) {
        ...
        if (me->getItem(quest_file, key[i]) >= value[i])
            msg += sprintf(
                ...
                HIK + ob->name(),
                HIK + ob->query("id"),
                HIK + me->getItem(quest_file, key[i]) + "/" + value[i]
            );
        else
            msg += sprintf(
                "  %s(%s)： %d/%d\n",
