...
int main(object me, string arg) {
    ...
    if (objectp(old = me->query_temp("pending/swear")) &&
        ...
        } else if (old->query_temp("pending/answer/" + me->query("id"), 1))
    ...
    if (stringp(me->query("born_family") != "没有") && me->query("born_family") == ob->query("born_family")) {
...
int do_right(object me, object ob) {
    ...
    switch (random(3)) {
        case 0:
            msg = "听说" + me->name(1) + "已和" + ob->name(1) + "结为异姓骨肉，共闯江湖。";
            break;
        case 1:
            msg = "据说" + me->name(1) + "和" + ob->name(1) + "一见如故，已经结为异姓骨肉。";
            break;
        default:
            msg = "听说" + me->name(1) + "与" + ob->name(1) + "义结金兰，携手行走江湖。";
            break;
    }
