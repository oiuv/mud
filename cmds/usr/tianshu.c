...
int main(object me, string arg)
{
    if (!arg)
    {
        ...
        else if (me->query(me->query("tianshu")))
...
void get_num(string arg, object me)
{
    ...
    {
        me->set("tianshu", "射雕英雄传");
        s = "4";
    }
    else if (arg[0] == '5')
    {
        me->set("tianshu", "白马啸西风");
        s = "5";
    }
    else if (arg[0] == '6')
    {
...
void renwu(object me)
{
        ...
        if (sss)
        {
            if (sss["class"] == "kill")
                tell_object(me, "你目前任务：帮" + me->query("xiansuo") +
                                "杀了" + sss["name"] + "。\n");
            else if (sss["class"] == "find")
                tell_object(me, "你目前任务：帮" + me->query("xiansuo") +
                                "找到" + sss["name"] + "。\n");
            else if (sss["class"] == "song")
                tell_object(me, "你目前任务：帮" + me->query("xiansuo") +
                                "把" + me->query("target") + "送到" + sss["name"] + "手中。\n");
            else if (sss["class"] == "baohu")
    ...
    else
    {
        ...
        if (me->query("特殊"))
        {
            ...
            {
            case "kill":
                tell_object(me, me->query("xiansuo") + "说道：你帮我杀了" + words[2] + "，带其尸体来见我！\n");
                tell_object(me, me->query("xiansuo") + "说道：" + words[3] + "\n");
                sss["class"] = "kill";
                sscanf(words[4], "%d", sss["exp"]);
                sss["name"] = words[2];
                break;

            case "find":
                tell_object(me, me->query("xiansuo") + "说道：你帮我找到" + words[2] + "！\n");
                tell_object(me, me->query("xiansuo") + "说道：" + words[3] + "\n");
                sss["class"] = "find";
                sscanf(words[4], "%d", sss["exp"]);
                sss["name"] = words[2];
            ...
            case "song":
                tell_object(me, me->query("xiansuo") + "说道：你帮我把" + words[3] + "送到" + words[2] + "手中！\n");
                tell_object(me, me->query("xiansuo") + "说道：" + words[4] + "\n");
                sss["class"] = "song";
                sscanf(words[5], "%d", sss["exp"]);
                sss["name"] = words[2];
...
void nextstep(object me)
{
    ...
    if (me->query("bu") >= random(3) + 9)
    {
        ...
        if (me->query(me->query("tianshu") + "s"))
            me->add(me->query("tianshu") + "s", 1);
        else
            me->set(me->query("tianshu") + "s", 1);
        me->delete ("xiansuo");
        me->delete ("特殊");
        me->delete ("心愿");
        me->delete ("bu");
...
{
    mapping renwu;
    if (!me->query_temp("jiaochai"))
        return;
    if (!me->query("心愿"))
    {
        tell_object(me, "你目前并无任务，你还是先找找线索。\n");
        return;
    }
    renwu = me->query("心愿");
    if (renwu["class"] == "kill")
        check_corpse(me);
    else if (renwu["class"] == "find")
        check_thing(me);
    else if (renwu["class"] == "song")
        check_song(me);
    else if (renwu["class"] == "baohu")
        check_baohu(me);
}
void check_thing(object me)
{
    ...
    while (i--)
    {
        if (inv[i]->query("name") == (me->query("心愿"))["name"])
            j = i;
    }
    if (j >= 0)
    {
        tell_object(me, me->query("xiansuo") + "说道：很好！你已经帮我找到" + (me->query("心愿"))["name"] + "了。\n");
        destruct(inv[j]);
        if (me->query("bu"))
            me->add("bu", 1);
        else
...
void check_baohu(object me)
{
    ...
    if (me->query("baohuren"))
    {
        tell_object(me, me->query("xiansuo") + "说道：我要你保护我。你还没完成！\n");
        return;
    }
    if (!me->query("baohu"))
    {
        tell_object(me, me->query("xiansuo") + "说道：我要你保护我。你完成了吗？\n");
        return;
    }
    else
    {
        tell_object(me, me->query("xiansuo") + "说道：很好！你已经保护过我。\n");
        me->delete ("baohu");
        if (me->query("bu"))
            me->add("bu", 1);
        else
...
void check_song(object me)
{
    ...
    if (!me->query("wancheng"))
    {
        tell_object(me, me->query("xiansuo") + "说道：我要你给我送东西。你完成了吗？\n");
        return;
    }
    else
    {
        tell_object(me, me->query("xiansuo") + "说道：很好！你已经完成任务了。\n");
        me->delete ("wancheng");
        if (me->query("bu"))
            me->add("bu", 1);
        else
...
void songwu(object me)
{
    ...
    while (i--)
    {
        if (inv[i]->query("name") == renwu["name"] && !userp(inv[i]))
            j = i;
    }
    if (j < 0)
    {
        tell_object(me, "你应该把" + me->query("target") + "送给" + renwu["name"] + "\n");
        return;
    }
    ...
    {
        tell_object(me, renwu["name"] + "皱了皱眉说道：这个，不对吧。\n");
        return;
    }
}
void reward(int exp, object me)
