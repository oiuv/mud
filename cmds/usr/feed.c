...
int main(object me, string arg)
{
    ...
    {
    case 0:
        message_vision(HIM "天空现出五彩祥云，天蓬元帅缓缓落下，一把拎起" HIM "$N" HIM "，冷笑两声，架云去了。\n" NOR,
                       ob);
        break;
    case 1:
        message_vision(HIM "天蓬元帅出现在一股烟雾中，一把抓住$N" HIM "，喝道：走，咱们家有事商量，快和我去！\n" NOR,
                       ob);
        break;
    case 2:
        message_vision(HIM "天蓬元帅急急忙忙的跑来，大声喊道：" +
                       ob->name() + HIM "！嫦娥姐姐来了！$N" HIM "一听，慌忙跟在老猪屁股后面颠颠的跑了。\n" NOR,
                       ob);
        break;
    default:
    ...
    message("vision", "只听“扑通”一声，" + ob->name() + "掉进了猪圈，继续高声打着呼噜。\n",
            environment(ob), ob);
    write(HIC "你的养猪大法成功了，" + ob->name(1) +
          HIC "现在进猪圈享福去了\n" NOR);

    CHANNEL_D->do_channel(this_object(), "rumor", "听说" + me->name(1) + HIM "施展养猪大法，" + ob->name(1) + HIM "进猪圈继续发呆，长睡不醒。");
    return 1;
}
