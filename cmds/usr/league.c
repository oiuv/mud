...
int main(object me, string arg)
{
    ...
    if (time() - me->query_temp("scan_time") < 3 && !wizardp(me) && arg != "check")
    ...
    case "dismiss":
        ...
        if (me->query("id") != me->query("league/leader_id") && !wizardp(me))
    ...
    case "add":
        ...
        if (me->query("id") != me->query("league/leader_id") && !me->query("league/grant"))
    ...
    case "grant":
        ...
        if (!arg ||
            ...
            lvl < 0 ||
            lvl > 4)
            return notify_fail("指令格式： league grant [id] [权限等级(0--4)]。\n");
        ...
        if (me == ob)
            return notify_fail("你已经是领袖了！\n");

        // 根据 lvl 授予权限
        lvlold = ob->query("league/grant");
        ob->set("league/grant", lvl);

        CHANNEL_D->do_channel(me, "inter", HIY + me->name() + HIG " 修改了 " HIY + ob->name() + HIG " 的权限。  " HIG + lvls[lvlold] + "→" + lvls[lvl] + "\n" NOR);
    ...
    case "kick":
        ...
        if (me->query("id") != me->query("league/leader_id") && me->query("league/grant") < 3)
    ...
    case "kill":
        ...
        if (me->query("id") != me->query("league/leader_id") && !wizardp(me) && me->query("league/grant") < 2)
    ...
    case "set":
        ...
        sscanf(arg, "%s %d", arg, lvl);
        if (arg != "no_kill" && arg != "weiwang" && arg != "follow")
        {
    ...
    case "title":
        ...
        if (me->query("id") != me->query("league/leader_id") && !wizardp(me) && me->query("league/grant") < 4)
            return notify_fail("你没有足够权限！\n");
...
int show_league_info(object me, string arg)
{
    ...
    foreach (id in member)
    {
        ...
        {
            msg += sprintf(HIY "在线   " NOR WHT "经验：" HIC "%-9d " NOR WHT "阅历：" HIW "%-8d " NOR WHT "威望：" HIY "%-8d\t" NOR,
                           ob->query("combat_exp"),
                           ob->query("score"),
                           ob->query("weiwang"));
...
int dismiss_league(object me, string arg)
{
    ...
    if (arg != me->query("league/league_name") && !wizardp(me))
    {
        write("你只能解散你所在的同盟。\n");
        return 1;
    }
...
int add_member(object me, string arg)
{
    ...
    if (!member->query_temp("wait_join"))
    {
        ...
        tell_object(member, "\n" + HIY + me->name() + "(" + me->query("id") + ")邀请你加入 「" HIG + me->query("league/league_name") + HIY " 」"
    ...
    {
        CHANNEL_D->do_channel(this_object(), "rumor", "听说" + member->name() + "(" + member->query("id") + ")加入了 「" HIC + me->query("league/league_name") + HIM " 」。\n" NOR);
        me->delete_temp("wait_reply");
        me->delete_temp("wait_join");
...
int league_kill(object me, string arg)
{
    ...
    // 战斗已经发生，队伍中所有的人参与战斗
    message("vision", HIR "你和大家一起跟着" + me->name(1) + HIR "冲了上去，围着" + obj->name() + "就是一顿乱砍。\n" NOR, l, ({me}));
