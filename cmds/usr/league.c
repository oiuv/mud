    ...
    switch (args[0]) {
        ...
        case "grant":
            ...
            CHANNEL_D->do_channel(
                me,
                "inter",
                HIY + me->name() + HIG " 修改了 " HIY + ob->name() + HIG " 的权限。  " HIG + lvls[lvlold] + "→" + lvls[lvl] + "\n" NOR
            );
...
int show_league_hatred(object me, string arg) {
    ...
    if (i < sizeof(ids))
        ...
        msg += WHT "目前一共是" + chinese_number(i) + "人。\n" NOR;
    write(msg);
    return 1;
}
...
int add_member(object me, string arg) {
    ...
    if (LEAGUE_D->add_member_into_league(me->query("league/league_name"), arg, me)) {
        CHANNEL_D->do_channel(
            this_object(),
            "rumor",
            "听说" + member->name() + "(" + member->query("id") + ")加入了 「" HIC + me->query("league/league_name") + HIM " 」。\n" NOR
        );
        me->delete_temp("wait_reply");
        me->delete_temp("wait_join");
...
int league_kill(object me, string arg) {
    ...
    message(
        "vision",
        HIR "你和大家一起跟着" + me->name(1) + HIR "冲了上去，围着" + obj->name() + "就是一顿乱砍。\n" NOR,
        l,
        ({ me })
    );
