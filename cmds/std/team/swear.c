...
int main(object me, string arg) {
    ...
    if (arg == "cancel") {
        if (arrayp(t = me->query_temp("pending/team_swear/member"))) {
            ...
            write("你取消和大家结义的念头。\n");
            t -= ({ 0 });
            message("vision", YEL + me->name(1) + "取消了大家一同结义的倡议。\n", t, me);
        } else
            write("你现在并没有倡议大家结义啊！\n");
    ...
    if (me->query("weiwang") < 1000) {
        ...
        return 1;
    } else if (me->query("weiwang") < 10000) {
        write("你这点名望还不够振臂一呼，聚帮结众，还是以后再说吧。\n");
        return 1;
    } else if (me->query("weiwang") < 20000) {
        write("你现在在江湖上也算是有些名头了，可惜还是不够。\n");
        return 1;
    } else if (me->query("weiwang") < 50000) {
...
int do_right(object me, object ob) {
    ...
    if (sizeof(t) == sizeof(r) && !sizeof(t - r) && !sizeof(r - t)) {
        ...
        switch (random(3)) {
            case 0:
                msg = "听说" + implode(t->name(1), "、") + "结成 「" HIG + league_name + HIM "」，纵横江湖！";
                break;
            case 1:
