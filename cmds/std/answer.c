...
int main(object me, string arg)
{
    ...
    if (me->query_temp("quest_gift/npc"))
    {
        ...
        if (objectp(obj) && environment(obj) == env && living(obj) && gift == 2)
        {
            message_vision(CYN "\n$N" CYN "急忙点点头，说"
                               "道：这东西正好我也需要，就交给"
                               "我吧。\n" NOR,
        ...
        else if (objectp(obj) && environment(obj) == env && living(obj) && gift == 1)
        {
            message_vision(CYN "\n$N" CYN "颇为尴尬的说道："
                               "嗯…我现在拿这东西也无用处，还"
                               "是算了吧。\n" NOR,
    ...
    if (!obj)
        obj = MESSAGE_D->find_user(target);
    ...
    message_vision(CYN "$N" CYN "回答$n" CYN "：『" HIG +
                   arg + NOR CYN "』\n" NOR,
                   me, obj);
