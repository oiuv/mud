...
int main(object me, string arg) {
    ...
    info = HIC + LOCAL_MUD_NAME() + "(" + HIW + upper_case(INTERMUD_MUD_NAME) + HIC + ")文章选集：\n" NOR WHT "------------------------------------"
        "--------------------------------------------------------\n" NOR;
        ...
        if (!arrayp(analectas) || !sizeof(analectas))
            ...
            if (!sizeof(analectas))
                ...
                info += read_analecta(
                    ...
                    member_array(
                        analectas[0],
                        ANALECTA_D->query_analecta_list()[sprintf("%d", year)]
                    ) + 1,
                    me
                );
        }
    ...
    if (arg && sscanf(arg, "add %d from %s", i, name) == 2) {
        if (!wizardp(me))
            info += CYN " 你尚无权力添加文选。\n" NOR;
        else if (!objectp(board = present(name, environment(me))))
            info += CYN " 这里没有 " WHT + name + CYN " 这块留言板。\n" NOR;
        else {
            ...
            if (!arrayp(all_note) || i < 1 || i > sizeof(all_note))
                ...
                info += CYN " 已经成功的添加了" + board->name() + "的 " WHT + (i + 1) + CYN " 号留言进入"
                    "本年度文选。\n" NOR;
            }
        }
    }
    // 删除文选
    else if (arg && sscanf(arg, "del %d %d", year, i) == 2) {
        if (!wizardp(me))
            info += CYN " 你尚无权力删除文选。\n" NOR;
        else if (!ANALECTA_D->delete_analecta(year, i))
            info += CYN " 没有 " WHT + year + CYN " 这个年度的文选"
                "或该年度没有 " WHT +
                i + CYN " 这个序号的文选"
                "。\n" NOR;
        else
            info += CYN " 年度 " WHT + year + CYN " 编号 " WHT + i + CYN " 的文选已经被成功删除。\n" NOR;
    } else  // 转换旧版本文选
    ...
    // 阅读指定年份的文章
    else if (arg && sscanf(arg, "%d %d", year, i) == 2) {
        analectas = ANALECTA_D->query_analecta_list()[sprintf("%d", year)];
        if (!arrayp(analectas) || !sizeof(analectas))
            info += CYN " 没有 " WHT + year + CYN " 这个年"
                "度的文选。\n" NOR;
        else if (sizeof(analectas) < i)
            info += CYN " 年度 " WHT + year + CYN " 没有 " WHT + i + CYN " 这个序号的文选。\n" NOR;

        else
            info += read_analecta(year, analectas[i - 1], i, me);
    }
    // 阅读本年度文章或者列出其他年度文章
    else if (arg && sscanf(arg, "%d", i)) {
        year = localtime(time())[LT_YEAR];
        ...
        } else if (arrayp(ANALECTA_D->query_analecta_list()[sprintf("%d", year)]) &&
            sizeof(analectas) && sizeof(analectas) >= i)
            info += read_analecta(year, analectas[i - 1], i, me);

        else
            info += CYN " 没有 " WHT + i + CYN " 这个年度或者"
                "本年这个序号的文选。\n" NOR;
    }
    // 列出有文选的年份
    else if (!arrayp(analectas) || !sizeof(analectas)) {
...
string read_analecta(int year, mapping analecta, int i, object me) {
    ...
    info = sprintf(CYN " [" WHT "%3d" CYN "] %-36s " WHT "%15s [ " CYN "%s " WHT "|" CYN
        " %s" WHT " ]\n\n" NOR,
        i, analecta["subject"],
        analecta["author_name"] + "-" + analecta["author_id"],
        ctime(analecta["time"])[0..9], analecta["board"]);
