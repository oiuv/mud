...
int look_room(object me, object env, int brief) {
    ...
    if (!brief) {
        ...
        str = sprintf(HIC + "%s" + NOR + "%s\n" + NOR,
            env->short(), wizardp(me) ? " - " + env->coordinate() : "");
    }
...
string look_equiped(object me, object obj, string pro) {
    ...
    if (objectp(hob = obj->query_temp("handing"))) {
        ...
        str = pro + "手中" + (mad ? "却" : "") + "握着一" + (hob->query_amount() ? hob->query("base_unit") : hob->query("unit")) + hob->name() + (mad ? "，疯了，一定是疯了！\n" : "。\n") + str;
    }
...
int look_living(object me, object obj) {
    ...
    if (obj->is_character() && obj->query("tattoo")) {
        ...
        if ((obj->query("tattoo/chest_long") == obj->query("tattoo/back_long"))
        ...
        if ((obj->query("tattoo/leftarm_long") == obj->query("tattoo/rightarm_long"))
            && obj->query("tattoo/leftarm_long")
            && obj->query("tattoo/rightarm_long")) {
