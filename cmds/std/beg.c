...
int main(object me, string arg)
{
    ...
    if (what != "cash" && what != "gold" && what != "silver" && what != "coin")
    ...
    if (!ob)
        ob = present("cash", victim);
    if (!ob)
        ob = present("gold", victim);
    if (!ob)
        ob = present("silver", victim);
    if (!ob)
        ob = present("coin", victim);
...
private void compelete_beg(object me, object victim, object ob, int sp, int dp)
{
    ...
    else
    {
        ...
        message_vision(CYN "$n" CYN "扭过头去，冷哼一声，对$N" CYN "理都不理。\n" NOR, me, victim);
        me->start_busy(3);
    }
}
