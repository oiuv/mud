...
int main(object me, string arg) {
    ...
    message("vision", WHT "只见" + me->name() + WHT "陪着笑脸，跟" + man->name() + WHT "说着话，好象在打听些什么。"
        "\n" NOR, environment(me), ({ me, man }));

    sp = me->query_skill("checking") * 10 + me->query("jing") * 3;

    dp = ob->query("jing") * 2;

    if (userp(ob) && me->query("combat_exp") <
        ob->query("combat_exp") * 2 / 3)
        return notify_fail(CYN + man->name() + CYN "皱着眉道：那" + ob->name() + CYN "比你强多了，你没事"
            "去招惹他做甚？\n" NOR);
    ...
    msg = HIG + man->name() + HIG "悄悄告诉你：" + ob->name() + HIG;
    ...
    msg += "修炼过" + to_chinese(sname[i]) + "，估计修炼到" + chinese_number(lvl) + "级了吧。\n" NOR;
