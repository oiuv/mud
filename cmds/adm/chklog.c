...
int main(object me, string arg) {
    ...
    foreach (chk in f_line)  //chk log file each line
    {
        if (sscanf(
            chk,
            "%s line %d: Warning: Unused local variable '%s'",
            filename,
            line_n,
            argv_name
        ) == 3) {
            ...
            while (line_n > 0) {
                ...
                if (isdata(wf_line[line_n])) {  //write("line_n number=="+(line_n+1)+"    argv_name is "+argv_name +"\n");
                    start = 0;
            ...
            chk,
            "%s line %d: Cannot #include %s before",
            filename,
            line_n,
            argv_name
        ) == 3) {
            ...
            while (line_n > 0) {
                line_n--;
                if (strsrch(wf_line[line_n], "<" + argv_name + ">") != -1) {  //write("line_n number=="+(line_n+1)+"    argv_name is "+argv_name +"\n");
                    //start=0;
                    check1();  //找到并寻找文件尝试修改
                    break;
                }
...
int check(int s)  //对这一行详细分析, 是否有未使用变量并修改
{
    ...
    if ((temp = strsrch(wf_line[line_n][s..], argv_name)) != -1)  //检查前后字符，看是否有这个变量
    {
        ...
        while (1) {
            if (peidui(argv_name) == 1) {                                            //write("right argv_name is "+argv_name +" c2 is "+c2+"\n");
                if (c2 == ";" || c2 == "/" || c2 == ",")  //一定要结束
                {
        ...
        wf_line[line_n] = wf_line[line_n][0..start - 1] + replace_string(
            wf_line[line_n][start..],
            argv_name,
            "",
            1
        );  //注释掉变量
