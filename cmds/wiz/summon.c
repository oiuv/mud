...
int main(object me, string arg)
{
    ...
    // moving
    tell_room(environment(ob), HIM "天空中伸出一只大手把" + (string)ob->query("name") + HIM "抓了起来, 然後不见了。\n" NOR, ob);
    tell_object(ob, "一只手把你抓了起来, 你眼前一阵黑....\n");
    tell_object(me, HIM "你把" + ob->name() + HIM "抓到你的面前。\n" NOR);
    tell_object(ob, HIM ".... 醒来时发现是" + me->name() + HIM "把你弄过来的。\n" NOR);
    tell_room(environment(me), HIM + ob->name() + HIM "突然出现" + "在你眼前。\n" NOR, ({me, ob}));
