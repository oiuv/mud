    ...
    if (weapon = me->query_temp("weapon")) {
        if (weapon->query("skill_type") == "pin") {
            if (me->query_skill("sword") < 100)
                ...
                msg = WHT "$N" WHT "轻弹出手中" + weapon->name() + WHT "，勾勒出一道绚丽的弧芒，将$n" WHT "的" + part[NAME] + WHT "割了下来。\n" NOR;
        } else if (weapon->query("skill_type") == "hammer") {
            if (me->query_skill("hammer") < 100)
                return notify_fail("你现在的锤法修为尚且无法进行切割。\n");
            else
                msg = WHT "$N" WHT "蓦地挥动" + weapon->name() + WHT "，听得「哐咚」一声闷响，已将$n" WHT "的" + part[NAME] + WHT "砸了下来。\n" NOR;
        } else if (weapon->query("skill_type") == "staff") {
            if (me->query_skill("staff") < 100)
                return notify_fail("你现在的杖法修为尚且无法进行切割。\n");
            else
                msg = WHT "$N" WHT "反转过手中" + weapon->name() + WHT "，只听「嗤啦」一声，已将" + part[NAME] + WHT "从$n" WHT "上割了下来。\n" NOR;
        } else if (weapon->query("skill_type") == "club") {
            if (me->query_skill("club") < 100)
                return notify_fail("你现在的棍法修为尚且无法进行切割。\n");
            else
                msg = WHT "$N" WHT "高举过手中" + weapon->name() + WHT "凌空劈下，随着「噗嗤」一声，已将" + part[NAME] + WHT "从$n" WHT "上劈了下来。\n" NOR;
        } else if (weapon->query("skill_type") == "whip") {
            if (me->query_skill("whip") < 100)
                return notify_fail("你现在的鞭法修为尚且无法进行切割。\n");
            else
                msg = WHT "$N" WHT "一振手中" + weapon->name() + WHT "，顿时只听「噼」的一声脆响，已将$n" WHT "的" + part[NAME] + WHT "卷了下来。\n" NOR;
        } else
            msg = WHT "$N" WHT "提起手中" + weapon->name() + WHT "「嗤」的一声便将$n" WHT "的" + part[NAME] + (stringp(part[VERB]) ? part[VERB] : WHT "割了下") + WHT "来。\n" NOR;
        ...
        msg = WHT "$N" WHT "举起手来，一下子就把$n" WHT "的" + part[NAME] + WHT "给" + (stringp(part[VERB]) ? part[VERB] : "切了下") + WHT "来。\n" NOR;
