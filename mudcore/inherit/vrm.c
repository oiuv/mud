 ...
 * 和游戏的可玩性。
 * 此对象目前适合于随机的迷宫，即：迷宫内房间的描述基
 * 本相同，比如一片树林、一片坟地等，如要此对象创作完
 * 整的随机区域即有一定的情节、一定格局的区域，则需要
 * 根据自己的情况规定出迷宫内房间描述的一些规则，使相
 * 邻房间的描述变化合理，房间内对象与描述协调。如果愿
...
// 只要能与其他房间相连的房间就肯定有一个入口.而可能的出口有三个.
