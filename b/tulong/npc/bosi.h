void init()
{
    ...
    if (! interactive(me) || me->is_fighting() || ! living(this_object()))
        return;

    if (me->query("family/family_name") == "明教")
        return;

    if (me->query_temp("have_yitianjian") || present("yitian jian", me))
    {
    ...
    if (me->query_temp("have_tulongdao") || present("tulong dao", me))
    {
